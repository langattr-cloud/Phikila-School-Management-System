...
def upgrade() -> None:
    ...
    if "tt_constraints" in tables:
        bind.execute(sa.text("DELETE FROM tt_constraints WHERE kind IN ('morning_preference', 'time_preference')"))
...
def downgrade() -> None:
    ...
    if "prefers_morning" not in columns:
        op.add_column("tt_subjects", sa.Column("prefers_morning", sa.Boolean(), nullable=False, server_default=sa.false()))

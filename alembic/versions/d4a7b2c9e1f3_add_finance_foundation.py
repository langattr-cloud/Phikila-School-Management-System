"""Reconcile finance accounting foundation and payment inbox."""
...
def upgrade() -> None:
    ...
    required = {"chart_of_accounts", "finance_journals", "finance_journal_entries", "payment_inbox"}
    ...
    if "finance_journals" not in tables:
        op.create_table("finance_journals",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("school_id", sa.Integer(), nullable=False),
            sa.Column("journal_number", sa.String(50), nullable=False), sa.Column("transaction_date", sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.Column("description", sa.Text(), nullable=False), sa.Column("reference", sa.String(100)),
            sa.Column("status", sa.String(20), server_default="posted"), sa.Column("created_by", sa.String(64)),
    ...
    if "finance_journal_entries" not in tables:
        op.create_table("finance_journal_entries",
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("journal_id", sa.Integer(), sa.ForeignKey("finance_journals.id", ondelete="CASCADE"), nullable=False),
    ...
    if "payment_inbox" not in tables:
        op.create_table("payment_inbox",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("school_id", sa.Integer(), nullable=False),
            sa.Column("source", sa.String(30), nullable=False), sa.Column("source_account", sa.String(100)), sa.Column("account_name", sa.String(200)),
            sa.Column("raw_message", sa.Text(), nullable=False), sa.Column("amount", sa.Numeric(12, 2), nullable=False),
            sa.Column("external_reference", sa.String(100), nullable=False), sa.Column("student_identifier", sa.String(50)),
            sa.Column("received_at", sa.DateTime(timezone=True), nullable=False), sa.Column("payment_channel", sa.String(50)),
            sa.Column("matched_student_id", sa.Integer(), sa.ForeignKey("students_v2.id")), sa.Column("match_method", sa.String(50)),
            sa.Column("match_confidence", sa.Numeric(5, 2)), sa.Column("status", sa.String(30), nullable=False, server_default="RECEIVED"),
            ...
            sa.Column("reviewed_by", sa.String(64)), sa.Column("reviewed_at", sa.DateTime(timezone=True)), sa.Column("posted_at", sa.DateTime(timezone=True)),
        ...
        for name, cols in {
            ...
            "ix_payment_inbox_matched_student_id": ["matched_student_id"], "ix_payment_inbox_status": ["status"],
            "ix_payment_inbox_posted_payment_id": ["posted_payment_id"],
        }.items():

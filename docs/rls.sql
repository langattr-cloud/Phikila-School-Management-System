...
-- ---------------------------------------------------------------------------
-- Helper: the schools the current Supabase user belongs to.
-- ---------------------------------------------------------------------------
...
-- A scheduler or admin may write; everyone else in the school reads only.
  ...
  foreach t in array tenant_tables loop
    ...
    execute format($f$
      create policy %I on public.%I
        ...
        with check (school_id in (select public.tt_user_schools())
                    and public.tt_can_write(school_id))
    $f$, t || '_write', t);
  end loop;
end
...
create policy tt_schools_write on public.tt_schools
  for all
  using (public.tt_user_role(id) in ('admin', 'super_admin'))
  with check (public.tt_user_role(id) in ('admin', 'super_admin'));

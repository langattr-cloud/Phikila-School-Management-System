...
export function PublishedTimetableGridWithEvents({view,mode,events}:Props){
 ...
 return <div className="timetable timetable--published timetable--final timetable--entity timetable--personal">
  ...
  <div className="entity-timetable-grid" style={{'--tt-period-count':periodCount,'--tt-day-count':dayCount,'--tt-column-template':columnTemplate,gridTemplateRows:rowTemplate,gridTemplateColumns:columnTemplate} as React.CSSProperties}>
   ...
   {days.flatMap((day,row)=>periods.map((period,column)=>{if(period.kind==='gap')return <div className="entity-timetable-cell entity-timetable-cell--gap" style={{gridColumn:column+2,gridRow:row+2}} key={`${day.index}-gap-${period.start_time}`}/>;const lesson=view.lessons.find(item=>item.day===day.index&&item.period===period.index);const event=events.find(item=>item.day_indexes.includes(day.index)&&item.start_time===period.start_time&&item.end_time===period.end_time);const isBreak=!lesson&&(!period.is_teaching||Boolean(event));const colour=lesson?validColour(lesson.subject_colour):undefined;return <div className={`entity-timetable-cell ${isBreak?'entity-timetable-cell--break':''}`} style={{gridColumn:column+2,gridRow:row+2}} key={`${day.index}-${period.index}`}>{lesson?<div className="entity-lesson-card" style={{'--subject-colour':colour??'#0F2A47'} as React.CSSProperties}><strong>{lesson.subject}</strong><span>{mode==='class'?(lesson.teacher||'—'):compactClass(lesson.class)}</span></div>:event?<span className="entity-break-letter" aria-label={`${event.name} ${event.start_time}–${event.end_time}`}>{eventLetter(event,day.index)}</span>:null}</div>}))}
  </div>

...
const PRESETS: Record<string, Partial<Appearance>> = {
  'Clean school': {
    lesson: { ...BEST_DEFAULT.lesson },
    period: { ...BEST_DEFAULT.period },
    day: { ...BEST_DEFAULT.day },
  },
  'High contrast': {
    lesson: { ...BEST_DEFAULT.lesson, background: '#0f172a', color: '#ffffff', size: 13 },
    period: { ...BEST_DEFAULT.period, background: '#e2e8f0', color: '#0f172a', size: 11 },
    day: { ...BEST_DEFAULT.day, background: '#cbd5e1', color: '#0f172a', size: 11 },
  },
  'Soft print': {
    lesson: { ...BEST_DEFAULT.lesson, background: '#ffffff', color: '#0f172a', size: 11 },
    period: { ...BEST_DEFAULT.period, background: '#ffffff', color: '#475569', size: 9 },
    day: { ...BEST_DEFAULT.day, background: '#f8fafc', color: '#475569', size: 9 },
  },
}
...
export function TimetableAppearanceEditor({ open, onClose, selectedCell }: { open: boolean; onClose: () => void; selectedCell?: SelectedCell | null }) {
  ...
  const selectionText = useMemo(() => {
    if (!selected) return 'Nothing selected yet. Click a lesson, period/time header, or day/date header in the timetable.'
  ...
  function applyPreset(name: string) {
    ...
    if (!preset) return
    setAppearance(value => ({
      lesson: { ...value.lesson, ...(preset.lesson || {}) },
      period: { ...value.period, ...(preset.period || {}) },
      day: { ...value.day, ...(preset.day || {}) },
    }))
  }
  ...
  return <div className="modal-backdrop" role="presentation" onClick={onClose}>
    <div className="card modal timetable-appearance-modal" role="dialog" aria-modal="true" aria-labelledby="timetable-appearance-title" onClick={event => event.stopPropagation()}>
      <div className="timetable-appearance-head">
        <div><div className="timetable-appearance-eyebrow">PRINT DESIGN</div><h2 id="timetable-appearance-title">Timetable Appearance</h2><p>Use the timetable itself as the selector, then edit the selected cell type and compare the complete design before saving.</p></div>
      ...
      <div className="timetable-appearance-selection">
        <div><span className="timetable-appearance-selection-label">Selected</span><strong>{selectionText}</strong></div>
        <label><span>Format</span><select className="input" value={cellType} onChange={event => setCellType(event.target.value as CellType)}><option value="lesson">Lesson / Subject card</option><option value="period">Period / Time header</option><option value="day">Day / Date header</option></select></label>
      </div>

      <div className="timetable-appearance-presets" aria-label="Design presets">
        <span>Quick design</span>
        {Object.keys(PRESETS).map(name => <button key={name} type="button" className="button button--ghost" onClick={() => applyPreset(name)}>{name}</button>)}
      </div>
      ...
      <div className="timetable-appearance-layout">
        <div className="timetable-appearance-controls">
          ...
          {advanced && <div className="timetable-appearance-more"><span>Selected type: <strong>{cellType}</strong></span><span>Changes are staged locally until Save Appearance.</span><span>Best Default restores only the selected type.</span></div>}
        </div>

        <div className="timetable-appearance-preview-wrap">
          <div className="timetable-appearance-preview-heading"><span>Live print preview</span><small>All three cell types</small></div>
          <div className="timetable-appearance-preview-grid">
            <button type="button" className={`timetable-appearance-preview-box ${cellType === 'day' ? 'is-selected' : ''}`} onClick={() => setCellType('day')}><span>DAY / DATE</span><div className="preview-day" style={previewStyle('day')}>Monday<br /><small>31 Aug</small></div></button>
            <button type="button" className={`timetable-appearance-preview-box ${cellType === 'period' ? 'is-selected' : ''}`} onClick={() => setCellType('period')}><span>PERIOD / TIME</span><div className="preview-period" style={previewStyle('period')}>Period 1<br /><small>08:00–09:00</small></div></button>
            <button type="button" className={`timetable-appearance-preview-box ${cellType === 'lesson' ? 'is-selected' : ''}`} onClick={() => setCellType('lesson')}><span>LESSON / SUBJECT</span><div className="preview-lesson" style={previewStyle('lesson')}>MATHEMATICS<br /><small>Mr. Smith</small></div></button>
          </div>
        </div>
      </div>

      <div className="timetable-appearance-footer"><button type="button" className="button button--ghost" onClick={restoreDefault}>Best Default</button><span className="timetable-appearance-scope">Applies to all matching cells in timetable views and printouts</span><div><button type="button" className="button button--ghost" onClick={onClose}>Cancel</button><button type="button" className="button button--primary" onClick={save}>Save Appearance</button></div></div>
    </div>
  </div>
}

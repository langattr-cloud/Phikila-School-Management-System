          ...
          {wholeRows.map((row) => (
            <div key={row.id} className="timetable__whole-row">
              ...
              {activeDays.flatMap((day) => teachingPeriods.map((period, index) => {
                ...
                return <div key={`${row.id}-${day.index}-${period.index}`} className={`timetable__whole-slot ${period.is_teaching ? '' : 'timetable__whole-slot--break'} ${currentSlot?.day === day.index && currentSlot.period === period.index ? 'timetable__whole-slot--now' : ''} ${hovered === key && Boolean(dragging || carrying) ? 'timetable__cell--target' : ''}`} aria-label={`${timetableClassLabel(row)}, ${dayLabel(day)}, period ${index + 1}`} tabIndex={readOnly ? -1 : 0} onClick={() => { if (cellLessons[0]) onSelect?.(cellLessons[0]); selectAppearanceCell('period', `Period ${index + 1}`, { day: day.index, period: period.index }) }} onKeyDown={(event) => handleCellKeyDown(event, day.index, period.index, cellLessons)} {...period.is_teaching ? slotHandlers(key, day.index, period.index) : {}}>{period.is_teaching && cellLessons.map(renderCard)}</div>
              }))}
            </div>
          ))}
        </div>

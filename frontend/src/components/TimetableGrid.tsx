...
export function TimetableGrid({
  ...
  const renderCard = (lesson: Lesson, preview = false) => {
    ...
    return (
      <div
        key={lesson.id}
        className={`lesson-card ${preview ? 'lesson-card--drag-preview' : ''} ${dragging?.id === lesson.id && !preview ? 'lesson-card--dragging' : ''} ${duration > 1 ? 'lesson-card--multi' : ''} ${contiguousSpan ? '' : 'lesson-card--invalid-span'} ${selectedId === lesson.id ? 'lesson-card--selected' : ''} ${conflict ? 'lesson-card--conflict' : ''} ${lesson.is_locked ? 'lesson-card--locked' : ''}`}
        style={style}
        title={title}
        aria-label={title}
        role="button"

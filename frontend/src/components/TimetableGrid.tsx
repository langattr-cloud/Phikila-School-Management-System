...
export function TimetableGrid({ days, periods, lessons, meta, conflicted, selectedId, readOnly = false, zoom = 1, dense = false, currentSlot, onSelect, onMove, onResize, onDropUnassigned, secondary, teacherInitials = false }: Props) {
  ...
  const streamName = (lesson: Lesson) => { const cls = meta.classes.get(lesson.class_id) as (SchoolClass & { academic_stream?: string | null }) | undefined; return cls?.academic_stream?.trim() || cls?.stream?.trim() || cls?.name || 'Class' }
  const describe = (lesson: Lesson) => { const subject = meta.subjects.get(lesson.subject_id)?.code ?? meta.subjects.get(lesson.subject_id)?.name ?? 'Lesson'; const extra = streamName(lesson); return extra ? `${subject}, ${extra}` : subject }
  ...
  function renderCard(lesson: Lesson, compact = false) {
    ...
    const extra = streamName(lesson)
    const bad = conflicted?.has(lesson.id)
    const shownDuration = resizing?.lessonId === lesson.id ? resizing.duration : (lesson.duration ?? 1)
    // TimetablePage passes a secondary callback that returns null for a class view.
    // In that view the ASC-style cell should show subject code + teacher code.
    const isClassView = Boolean(secondary && secondary(lesson) === null)
    const teacherCode = teacher?.code?.trim() || teacher?.staff_number?.trim() || ''
    const showInitials = teacherInitials || (secondary && extra === null)
    const teacherLabel = isClassView ? teacherCode : (showInitials ? initials(teacher?.name) : teacher?.name)

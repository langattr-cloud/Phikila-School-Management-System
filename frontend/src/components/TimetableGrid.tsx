...
export function TimetableGrid({
  ...
  const renderCard = (lesson: Lesson) => {
    ...
    return (
      <div
        key={lesson.id}
        className={`lesson-card ${duration > 1 ? 'lesson-card--multi' : ''} ${contiguousSpan ? '' : 'lesson-card--invalid-span'} ${selectedId === lesson.id ? 'lesson-card--selected' : ''} ${conflict ? 'lesson-card--conflict' : ''} ${lesson.is_locked ? 'lesson-card--locked' : ''}`}
        style={style}
        title={title}
        aria-label={title}
        role="button"
  ...
  const renderDayPeriod = () => (
    <div className="timetable__time-grid" style={{ '--tt-period-count': periodsPerDay || 1 } as CSSProperties}>
      ...
      {activeDays.map((day) => <div key={day.index} className="timetable__day-row"><div className="timetable__day-label">{dayLabel(day)}</div>{displayPeriods.map((period) => { const key = `${day.index}:${period.index}`; const cellLessons = bySlot.get(key) ?? []; return <div key={period.index} className={`timetable__cell ${period.is_teaching ? '' : 'timetable__break-cell'} ${hovered === key && period.is_teaching ? 'timetable__cell--target' : ''}`} data-day={day.index} data-period={period.index} tabIndex={readOnly || !period.is_teaching ? -1 : 0} onKeyDown={(event) => handleCellKeyDown(event, day.index, period.index, cellLessons)} onContextMenu={(event) => { if (view === 'class' || view === 'teacher') { event.preventDefault(); event.stopPropagation(); const lesson = cellLessons[0]; if (lesson) setSelectedPrintLesson(lesson) } }} {...slotHandlers(key, day.index, period.index)}>{cellLessons.map(renderCard)}</div> })}</div>)}
    </div>
  )

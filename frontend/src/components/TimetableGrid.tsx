...
type Props = {
  ...
  onMoveMany?: (moves: Array<{ lesson: Lesson; day: number; period: number }>) => void
...
export function TimetableGrid({
  ...
  const moveLesson = (lesson: Lesson, day: number, period: number) => {
    ...
    if (moved.length > 1 && onMoveMany) onMoveMany(moved)
    else moved.forEach((item) => onMove?.(item.lesson, item.day, item.period))
    setDraggingGroup([])
    setDragging(null)
    setCarrying(null)
    setDraggingGroup([])
  ...
  const renderWholeSchool = () => {
    ...
    return (
      <div className="timetable__whole-scroll" style={{ zoom: zoom ? zoom / 100 : 1 }}>
  ...
  const renderDayPeriod = () => (
    <div className="timetable__time-grid" style={{ '--tt-period-count': periodsPerDay || 1, zoom: zoom ? zoom / 100 : 1 } as CSSProperties}>
  ...
  return (
    <div
      className={`timetable timetable--${view}-view${density}`}

...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => { document.body.classList.remove('body--locked'); document.removeEventListener('keydown', onKeyDown) }
  }, [drawerOpen])

  async function handleSignOut() {
    if (signingOut) return
    setSigningOut(true); const result = await signOut(); setSigningOut(false)
    if (!result.ok) { notify(result.message, 'error'); return }
    notify('You have been signed out.', 'success'); navigate('/login?notice=signed-out', { replace: true })
  }

  const navigation = <nav className="sidebar__nav" aria-label="Main">
    {groups.map((group) => <div className="sidebar__group" key={group.label}>
      <p className="sidebar__group-label">{group.label}</p>
      <ul className="sidebar__list">{group.items.map((item) => {
        const active = isActive(pathname, item.to)
        return <li key={item.to}><Link to={item.to} className={`sidebar__link ${active ? 'sidebar__link--active' : ''}`.trim()} aria-current={active ? 'page' : undefined}>
          {item.icon && <span className="sidebar__icon" aria-hidden="true">{item.icon}</span>}{item.label}
        </Link></li>
      })}</ul>
    </div>)}
  </nav>

  const accountBlock = <div className="sidebar__account">
    <p className="sidebar__account-name" title={displayName(user)}>{displayName(user)}</p>

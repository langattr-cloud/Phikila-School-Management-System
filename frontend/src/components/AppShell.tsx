...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    function onKeyDown(e: KeyboardEvent) {
      if (e.key === 'Escape') { setDrawerOpen(false); menuButtonRef.current?.focus(); return }
  ...
  const examinationGroup = (
    <div className="sidebar__group" key="Examinations">
      ...
      <ul className="sidebar__list">
        <li>
          <div className="sidebar__link" style={{ padding: 0 }}>
            <Link
              to="/examinations"
              className={isActive(pathname, '/examinations') ? 'sidebar__link--active' : undefined}
              aria-current={normalisePath(pathname) === '/examinations' ? 'page' : undefined}
              style={{ display: 'flex', alignItems: 'center', flex: 1, minWidth: 0, textDecoration: 'none', color: 'inherit', padding: 'var(--space-2) var(--space-3)' }}
            >
          ...
          {examinationsOpen && <ul className="sidebar__list" style={{ paddingLeft: '1rem' }}>{EXAMINATION_NAV.slice(1).map(renderNavItem)}</ul>}
        </li>
      </ul>
    </div>
  )

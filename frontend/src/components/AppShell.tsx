...
import { CalendarIcon, CheckIcon, CloseIcon, DashboardIcon, GridIcon, InboxIcon, LayersIcon, LogOutIcon, MenuIcon, MoonIcon, SchoolIcon, SparkIcon, StackedLayersIcon, SunIcon, UserIcon } from './icons'
...
const NAV: NavGroup[] = [
  ...
  { label: 'Scheduling', items: [{ to: '/scheduling/requirements', label: 'Teaching Allocations', icon: <LayersIcon /> }, { to: '/scheduling/constraints', label: 'Constraints', icon: <CheckIcon /> }, { to: '/scheduling/generate', label: 'Generate timetable', icon: <SparkIcon /> }, { to: '/scheduling/copilot', label: 'Copilot', icon: <SparkIcon /> }] },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  async function exportStudents() {
    try {
      const result = await apiFetchBlob('/api/v1/students/export')
      const url = URL.createObjectURL(result.blob)
      const anchor = document.createElement('a')
      anchor.href = url
      anchor.download = result.filename || `students-${new Date().toISOString().slice(0, 10)}.xlsx`
      ...
      notify(error instanceof Error ? error.message : 'Could not export students.', 'error')
    }
  }
    ...
    <div className="app-shell">
      ...
      <div className="app-shell__main">
        <header className="topbar">
          <button ref={menuButtonRef} type="button" className="icon-button topbar__menu" onClick={() => setDrawerOpen(true)} aria-label="Open navigation menu" aria-expanded={drawerOpen} aria-controls="mobile-navigation">
            <MenuIcon />
          </button>
          ...
          {completedJob?.status === 'completed' && <Link className="button button--primary button--sm" to="/timetable/whole-school" aria-label="Timetable ready, open whole-school timetable">Timetable ready →</Link>}
        ...
        <aside id="mobile-navigation" ref={drawerRef} className={`sidebar sidebar--drawer ${drawerOpen ? 'sidebar--open' : ''}`.trim()} role="dialog" aria-modal={drawerOpen || undefined} aria-label="Navigation menu" aria-hidden={!drawerOpen}>
          <div className="sidebar__brand">
            ...
            <button type="button" className="icon-button" onClick={() => { setDrawerOpen(false); menuButtonRef.current?.focus() }} aria-label="Close navigation menu"><CloseIcon /></button>
          </div>
          {navigation}
          {accountBlock}
        </aside>

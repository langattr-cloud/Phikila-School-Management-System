...
export function FinanceBanking() {
  ...
  const reconcile = async () => {
    ...
    try {
      ...
      setMessage(result.status === 'RECONCILED' ? 'Bank account reconciled.' : `Reconciliation saved with a KES ${Number(result.difference).toLocaleString()} difference.`)
      setNotes('')
    } catch (err) {
  ...
  return <section className="section card">
    ...
    {loading ? <LoadingBlock label="Loading banking" rows={5} /> : !accounts.length ? <EmptyState title="No bank accounts" description="Create a bank account before importing statements or reconciling balances." /> : <>
      <div className="finance-form"><div className="finance-form__grid">
        ...
        <div className="field"><label className="field__label">CSV Statement</label><input className="input" type="file" accept=".csv,text/csv" onChange={(e) => setFile(e.target.files?.[0] || null)} /></div>
      ...
      <div className="finance-form"><div className="finance-form__grid">
        ...
        <div className="field"><label className="field__label">Statement Balance (KES)</label><input className="input" type="number" step="0.01" value={statementBalance} onChange={(e) => setStatementBalance(e.target.value)} /></div>
        <div className="field"><label className="field__label">Book Balance (KES)</label><input className="input" type="number" step="0.01" value={bookBalance} onChange={(e) => setBookBalance(e.target.value)} /></div>
      ...
      {!transactions.length ? <EmptyState title="No bank transactions" description="Import a CSV statement to populate this account." /> : <div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Reference</th><th>Description</th><th>Amount</th><th>Status</th></tr></thead><tbody>{transactions.map((transaction) => <tr key={transaction.id}><td>{new Date(transaction.transaction_date).toLocaleDateString()}</td><td>{transaction.transaction_type}</td><td>{transaction.external_reference || '—'}</td><td>{transaction.description || '—'}</td><td className="number-cell">KES {Number(transaction.amount).toLocaleString()}</td><td><Badge tone={transaction.status === 'MATCHED' ? 'success' : 'warning'}>{transaction.status}</Badge></td></tr>)}</tbody></table></div>}

      <div className="finance-section-heading"><h3 className="section__title">Reconciliation History</h3></div>
      {!reconciliations.length ? <p className="muted-text">No reconciliations recorded.</p> : <div className="table-scroll"><table><thead><tr><th>Date</th><th>Account</th><th>Statement</th><th>Book</th><th>Difference</th><th>Status</th></tr></thead><tbody>{reconciliations.slice(0, 20).map((row) => <tr key={row.id}><td>{row.statement_date}</td><td>#{row.bank_account_id}</td><td className="number-cell">KES {Number(row.statement_balance).toLocaleString()}</td><td className="number-cell">KES {Number(row.book_balance).toLocaleString()}</td><td className="number-cell">KES {Number(row.difference).toLocaleString()}</td><td><Badge tone={row.status === 'RECONCILED' ? 'success' : 'warning'}>{row.status}</Badge></td></tr>)}</tbody></table></div>}
    </>}
  </section>
}

...
type CellType = 'lesson' | 'period' | 'day' | 'title'
type SelectedCell = { type: CellType; label: string; day?: number; period?: number }
  ...
  useEffect(() => {
    const handleContextMenu = (event: MouseEvent) => {
      ...
      if (title) {
        ...
        dispatchSelection({ type: 'title', label: title.textContent?.trim() || 'Timetable title' })
        return
      }
      if (lesson) {
        event.preventDefault()
        const label = lesson.querySelector('.lesson-card__subject')?.textContent?.trim() || lesson.getAttribute('aria-label') || 'Lesson'
        dispatchSelection({ type: 'lesson', label })
        return
      }
      if (day) {
        event.preventDefault()
        dispatchSelection({ type: 'day', label: day.textContent?.trim() || 'Day' })
        return
      }
      if (period) {
        event.preventDefault()
        dispatchSelection({ type: 'period', label: period.textContent?.trim() || 'Period' })
      }
    }

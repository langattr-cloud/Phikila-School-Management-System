...
type PrintValue = TitleSettings | DaySettings | PeriodSettings | LessonSettings
type PrintPatch = Partial<TitleSettings> | Partial<DaySettings> | Partial<PeriodSettings> | Partial<LessonSettings>
export type PrintStore = { get: (type: PrintTargetType, id: string) => PrintValue; set: (type: PrintTargetType, id: string, value: PrintPatch) => void; reset: (type: PrintTargetType, id: string) => void; state: PrintConfig }
...
export function usePrintStore(): PrintStore {
  const [state, setState] = useState<PrintConfig>(() => safeLoad())
  useEffect(() => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)) } catch {} }, [state])
  const get = (type: PrintTargetType, id: string): PrintValue => {
    if (type === 'title') return { prefix: false, prefixText: '', format: 'first_last', centered: true, header: '', ...(state.title[id] ?? {}) }
    if (type === 'day') return { format: 'full', position: DEFAULT_POSITION, vertical: false, overflow: 'smaller', ...(state.days[id] ?? {}) }
    if (type === 'period') return { showInterval: true, timePosition: 1, lessonPosition: 7, twoLines: false, showBell: false, ...(state.periods[id] ?? {}) }
    return { ...defaultLesson(), ...(state.lessons[id] ?? {}) }
  }
  const set = (type: PrintTargetType, id: string, value: PrintPatch) => setState((previous) => {
  ...
  const reset = (type: PrintTargetType, id: string) => setState((previous) => {
    const bucket = type === 'title' ? 'title' : type === 'day' ? 'days' : type === 'period' ? 'periods' : 'lessons'
    const next = { ...previous[bucket] }; delete next[id]
    return { ...previous, [bucket]: next } as PrintConfig
  })
...
type CommonPopupProps = { id: string; get: PrintStore['get']; set: PrintStore['set']; onClose: () => void }
export function TitlePopup({ id, get, set, onClose }: CommonPopupProps) {
  const value = get('title', id) as TitleSettings; const update = (patch: Partial<TitleSettings>) => set('title', id, patch)
...
export function DayPopup({ id, get, set, onClose }: CommonPopupProps) {
  const value = get('day', id) as DaySettings; const update = (patch: Partial<DaySettings>) => set('day', id, patch)
...
export function PeriodPopup({ id, get, set, onClose }: CommonPopupProps) {
  const value = get('period', id) as PeriodSettings; const update = (patch: Partial<PeriodSettings>) => set('period', id, patch)
...
export function LessonPopup({ id, get, set, onClose }: CommonPopupProps) {
  const value = get('lesson', id) as LessonSettings; const updateField = (fieldName: keyof LessonSettings, patch: Partial<FieldSettings>) => set('lesson', id, { [fieldName]: { ...value[fieldName], ...patch } } as Partial<LessonSettings>)
...
export function PrintSetup4({ initial, onClose }: { initial?: PopupState; onClose?: () => void }) {
  ...
  const props: CommonPopupProps = { id: popup.id, get: store.get, set: store.set, onClose: close }
  return <>{popup.type === 'title' && <TitlePopup {...props} />}{popup.type === 'day' && <DayPopup {...props} />}{popup.type === 'period' && <PeriodPopup {...props} />}{popup.type === 'lesson' && <LessonPopup {...props} />}</>
}
...
export function PrintSetupProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const onSelected = (event: Event) => {
      const detail = (event as CustomEvent<{ targetType?: PrintTargetType; type?: PrintTargetType; dayId?: number | string; periodId?: number | string; lessonId?: number | string }>).detail

...
const classPayload = (payload: Partial<SchoolClassInput> & Loose): SchoolClassInput => ({ name: String(payload.name ?? '').trim(), code: String(payload.code ?? '').trim(), grade: payload.grade == null || String(payload.grade).trim() === '' ? null : String(payload.grade).trim(), stream: payload.stream == null || String(payload.stream).trim() === '' ? null : String(payload.stream).trim(), student_count: Number.isFinite(Number(payload.student_count)) ? Number(payload.student_count) : 40, home_room_id: payload.home_room_id == null || String(payload.home_room_id).trim() === '' ? null : Number(payload.home_room_id), class_teacher_id: payload.class_teacher_id == null || String(payload.class_teacher_id).trim() === '' ? null : Number(payload.class_teacher_id), unavailable: payload.unavailable && typeof payload.unavailable === 'object' ? payload.unavailable as Slots : {}, })

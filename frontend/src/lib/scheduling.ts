...
export interface SchoolClass extends Loose { id: number; name: string; code: string; student_count?: number; home_room_id?: number | null; class_teacher_id?: number | null; unavailable?: Slots; level_id?: number | null; academic_year_id?: number | null }
export interface SchoolClassInput { name: string; code: string; student_count: number; home_room_id: number | null; class_teacher_id: number | null; unavailable: Slots; level_id?: number | null; academic_year_id?: number | null }
...
export interface Version extends Loose { id: number; number?: number; status: string; effective_from?: string | null; published_at?: string | null }
...
export interface AuditEntry extends Loose { id?: number; action?: string; entity?: string; entity_id?: number | null; summary?: string; actor?: string; at?: string }
...
const classPayload = (payload: Partial<SchoolClassInput> & Loose): SchoolClassInput => {
  ...
  const studentCount = Number.isFinite(rawStudentCount) ? Math.max(0, Math.floor(rawStudentCount)) : 0

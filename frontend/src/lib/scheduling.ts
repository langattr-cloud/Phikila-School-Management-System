...
export type GenerationMode = 'strict'|'draft'|'relax'
export type GenerationComplexity = 'fast'|'balanced'|'thorough'
export interface GenerateIn extends Loose { max_seconds?: number; timetable_type_id?: number | null; class_ids?: number[] | null; teacher_ids?: number[] | null; period_indexes?: number[] | null; mode?: GenerationMode; complexity?: GenerationComplexity; test_first?: boolean }
...
export interface GenerationTestResult extends Loose { passed:boolean; feasible:boolean; mode:GenerationMode; checks:Array<{key:string;label:string;state:string}>; problems:string[]; relaxed_constraints:string[]; quality?:Quality; stats?:Loose }
...
const ACTIVE_GENERATION_KEY='timetable:generation_job'
const rememberGenerationJob=(job:Job)=>{if(typeof window!=='undefined'&&job?.id)window.localStorage.setItem(ACTIVE_GENERATION_KEY,String(job.id));return job}
const forgetGenerationJob=()=>{if(typeof window!=='undefined')window.localStorage.removeItem(ACTIVE_GENERATION_KEY)}
export const scheduling = {
 ...
 generate:async(maxSeconds=30,p:GenerateIn={})=>rememberGenerationJob(await send<Job>('/solver/generate','POST',{max_seconds:maxSeconds,...p})), generateAsync:async(maxSeconds=30,p:GenerateIn={})=>rememberGenerationJob(await send<Job>('/solver/generate','POST',{max_seconds:maxSeconds,...p})), activeJob:async()=>{if(typeof window==='undefined')return null;const raw=window.localStorage.getItem(ACTIVE_GENERATION_KEY);const id=raw?Number(raw):NaN;if(!Number.isFinite(id)||id<=0)return null;try{const job=await get<Job>(`/solver/jobs/${id}`);if(['completed','failed','cancelled'].includes(String(job.status).toLowerCase())){forgetGenerationJob()}return job}catch(e){forgetGenerationJob();return null}}, generateProfile:async(p:GenerateProfileInput)=>rememberGenerationJob(await send<Job>('/solver/generate','POST',p)), testGeneration:(p:GenerateIn={})=>send<GenerationTestResult>('/solver/test','POST',p), job:(id:number)=>get<Job>(`/solver/jobs/${id}`), cancelJob:async(id:number)=>{const job=await send<Job>(`/solver/jobs/${id}/cancel`,'POST');return job}, versions:()=>get<Version[]>('/versions'), currentVersion:async()=>{const raw=new URLSearchParams(window.location.search).get('version_id');const requested=raw?Number(raw):NaN;if(Number.isFinite(requested)&&requested>0){const versions=await get<Version[]>('/versions');return versions.find(version=>version.id===requested)??null}try{return await get<Version|null>('/versions/current')}catch(e){if(e instanceof Error&&/404|not found/i.test(e.message))return null;throw e}}, lessons:(versionId:number)=>get<Lesson[]>(`/versions/${versionId}/lessons`), conflicts:async(versionId:number)=>(await get<Conflict[]>(`/versions/${versionId}/conflicts`)).filter(c=>c.kind!=='no_room'), publish:(versionId:number)=>send<Version>(`/versions/${versionId}/publish`,'POST'), restore:(versionId:number)=>send<Version>(`/versions/${versionId}/restore`,'POST'), deleteVersion:(versionId:number)=>send<void>(`/versions/${versionId}`,'DELETE'), moveLesson:(id:number,p:{day_index:number;period_index:number;room_id?:number|null})=>send<Lesson>(`/lessons/${id}`,'PATCH',p), patchLesson:(id:number,p:Loose)=>send<Lesson>(`/lessons/${id}`,'PATCH',p), duplicateLesson:(id:number)=>send<Lesson>(`/lessons/${id}/duplicate`,'POST'), deleteLesson:(id:number,p?:Loose)=>send<void>(`/lessons/${id}/delete`,'POST',p), createLesson:(versionId:number,p:Loose)=>send<Lesson>(`/versions/${versionId}/lessons`,'POST',p), unassigned:(versionId:number)=>get<Unassigned[]>(`/versions/${versionId}/unassigned`), assignRooms:(versionId:number)=>send<{assigned:number}>(`/versions/${versionId}/assign-rooms`,'POST'), explain:(id:number,day_index:number,period_index:number)=>send<Explanation>(`/lessons/${id}/explain?day_index=${day_index}&period_index=${period_index}`,'POST'), suggestions:(id:number)=>get<Alternative[]>(`/lessons/${id}/suggestions`), dashboard:()=>get<Dashboard>('/dashboard'), analytics:()=>get<Analytics>('/analytics'), audit:(limit=50)=>get<Loose[]>(`/audit?limit=${limit}`), amendments:async(limit=5)=>{try{return await get<TimetableAmendment[]>(`/amendments?limit=${limit}`)}catch(e){if(e instanceof Error&&/404|not found/i.test(e.message))return [];throw e}}, view:(scope:'class'|'teacher'|'room'|'subject',targetId:number,timetableTypeId?:number)=>get<TimetableView>(`/timetable/view?scope=${scope}&target_id=${targetId}${timetableTypeId ? `&timetable_type_id=${timetableTypeId}` : ''}`), interpret:(text:string)=>send<CopilotCommand>('/copilot/interpret','POST',{text}), applyCommand:(command:CopilotCommand)=>send<Loose>('/copilot/apply','POST',{command})
}

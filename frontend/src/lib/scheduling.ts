...
export interface TimetableDisplayPeriod { index:number; name:string; short_form?:string; start_time:string; end_time:string; is_teaching:boolean }
...
export const scheduling = {
 ...
 generate:(maxSeconds=30,p:GenerateIn={})=>send<Job>('/solver/generate-async','POST',{max_seconds:maxSeconds,...p}), generateAsync:(maxSeconds=30,p:GenerateIn={})=>send<Job>('/solver/generate-async','POST',{max_seconds:maxSeconds,...p}), activeJob:()=>get<Job|null>('/solver/jobs/active'), generateProfile:(p:GenerateProfileInput)=>send<Job>('/solver/generate-profile','POST',p), job:(id:number)=>get<Job>(`/solver/jobs/${id}`), cancelJob:(id:number)=>send<Job>(`/solver/jobs/${id}/cancel`,'POST'), versions:()=>get<Version[]>('/versions'), currentVersion:async()=>{try{return await get<Version|null>('/versions/current')}catch(e){if(e instanceof Error&&/404|not found/i.test(e.message))return null;throw e}}, lessons:(versionId:number)=>get<Lesson[]>(`/versions/${versionId}/lessons`), conflicts:async(versionId:number)=>{const result=await get<ConflictSummary>(`/versions/${versionId}/conflicts`); return result.conflicts.filter(c=>c.kind!=='no_room')}, validate:(versionId:number)=>get<ValidationSummary>(`/versions/${versionId}/validate`), publish:(versionId:number)=>send<Version>(`/versions/${versionId}/publish`,'POST'), restore:(versionId:number)=>send<Version>(`/versions/${versionId}/restore`,'POST'), deleteVersion:(versionId:number)=>send<void>(`/versions/${versionId}`,'DELETE'), moveLesson:(id:number,p:{day_index:number;period_index:number;room_id?:number|null})=>send<Lesson>(`/lessons/${id}`,'PATCH',p), patchLesson:(id:number,p:{day_index?:number;period_index?:number;duration?:number})=>send<Lesson>(`/lessons/${id}`,'PATCH',p), duplicateLesson:(id:number)=>send<Lesson>(`/lessons/${id}/duplicate`,'POST'), deleteLesson:(id:number,p?:Loose)=>send<void>(`/lessons/${id}/delete`,'POST',p), bulkDuplicateLessons:(versionId:number,lessonIds:number[])=>send<Lesson[]>(`/versions/${versionId}/lessons/bulk-duplicate`,'POST',{lesson_ids:lessonIds}), bulkDeleteLessons:(versionId:number,lessonIds:number[])=>send<void>(`/versions/${versionId}/lessons/bulk-delete`,'POST',{lesson_ids:lessonIds}), bulkLockLessons:(versionId:number,lessonIds:number[],locked:boolean)=>send<void>(`/versions/${versionId}/lessons/bulk-lock`,'POST',{lesson_ids:lessonIds,locked}), createLesson:(versionId:number,p:Loose)=>send<Lesson>(`/versions/${versionId}/lessons`,'POST',p), unassigned:(versionId:number)=>get<Unassigned[]>(`/versions/${versionId}/unassigned`), assignRooms:(versionId:number)=>send<{assigned:number}>(`/versions/${versionId}/assign-rooms`,'POST'), explain:(id:number,day_index:number,period_index:number)=>send<Explanation>(`/lessons/${id}/explain`,'POST',{day_index,period_index}), suggestions:(id:number)=>get<Alternative[]>(`/lessons/${id}/suggestions`), dashboard:()=>get<Dashboard>('/dashboard'), analytics:()=>get<Analytics>('/analytics'), audit:(limit=50)=>get<Loose[]>(`/audit?limit=${limit}`), amendments:async(limit=5)=>{try{return await get<TimetableAmendment[]>(`/amendments?limit=${limit}`)}catch(e){if(e instanceof Error&&/404|not found/i.test(e.message))return [];throw e}}, view:(scope:'class'|'teacher'|'room'|'subject',targetId:number,timetableTypeId?:number)=>get<TimetableView>(`/timetable/view?scope=${scope}&target_id=${targetId}${timetableTypeId ? `&timetable_type_id=${timetableTypeId}` : ''}`), interpret:(text:string)=>send<CopilotCommand>('/copilot/interpret','POST',{text}), applyCommand:(command:CopilotCommand)=>send<Loose>('/copilot/apply','POST',{command})
}
export function displayPeriods(periods:Period[]):Period[]{return [...periods].sort((a,b)=>a.index-b.index)}
export function teachingPeriods(periods:Period[]):Period[]{return displayPeriods(periods).filter(p=>p.is_teaching)}
export function breakPeriods(periods:Period[]):Period[]{return displayPeriods(periods).filter(p=>!p.is_teaching)}
...
export interface ConflictSummary extends Loose { total:number; hard:number; soft:number; conflicts:Conflict[] }
export interface ValidationSummary extends Loose { valid:boolean; hard_conflicts:number; soft_conflicts:number; unassigned_requirements:number; missing_periods:number; message:string }

...
export interface Requirement { id: number; class_id: number; class_name?: string; subject_id: number; subject_name?: string; teacher_id: number; teacher_name?: string; room_id?: number | null; room_name?: string | null; periods_per_week: number; double_periods: number }
export interface RequirementInput { class_id: number; subject_id: number; teacher_id: number; room_id?: number | null; periods_per_week: number; double_periods?: number }
...
export const scheduling = {
  me: () => get<Principal>('/me'), calendar: () => get<Calendar>('/calendar'), saveCalendar: (payload: { days: DayInput[]; periods: PeriodInput[] }) => send<Calendar>('/calendar', 'PUT', payload),

...
export interface StudentResult { student_id: number; student_name: string; admission_number: string; subject_scores: SubjectScore[]; total_score: number; average: number; position?: number; grade?: string; education_level?: 'primary' | 'junior' | 'senior' | null; percentage?: number; band?: string; band_label?: string; deviation?: number; progress?: number }
...
export interface GradeScale { id: number; school_id: number; grade: string; min_score: number; max_score: number; points: number | null; description: string | null; education_level: 'primary' | 'junior' | 'senior' | null }
export interface GradeScaleCreate { grade: string; min_score: number; max_score: number; points?: number | null; description?: string | null; education_level?: 'primary' | 'junior' | 'senior' | null }

type AcademicContext = { academic_year_id?: number; level_id?: number; grade_id?: number; stream_id?: number }
...
const contextQuery = (context: AcademicContext) => { const q = new URLSearchParams(); Object.entries(context).forEach(([k, v]) => { if (v != null) q.set(k, String(v)) }); return q.toString() }

export const examinations = {
  listSeries: () => get<ExamSeries[]>(`${BASE}/examinations/series`),
  createSeries: (payload: SeriesCreate) => send<ExamSeries>(`${BASE}/examinations/series`, 'POST', payload),
  updateSeries: (id: number, payload: SeriesUpdate) => send<ExamSeries>(`${BASE}/examinations/series/${id}`, 'PATCH', payload),
  list: (seriesId?: number) => get<Examination[]>(`${BASE}/examinations${seriesId != null ? `?series_id=${seriesId}` : ''}`),
  get: (id: number) => get<Examination>(`${BASE}/examinations/${id}`),
  create: (payload: ExaminationCreate) => send<Examination>(`${BASE}/examinations`, 'POST', payload),
  update: (id: number, payload: ExaminationUpdate) => send<Examination>(`${BASE}/examinations/${id}`, 'PATCH', payload),
  delete: (id: number) => send<void>(`${BASE}/examinations/${id}`, 'DELETE'),
  setStatus: (id: number, status: 'draft' | 'active' | 'published' | 'locked') => send<Examination>(`${BASE}/examinations/${id}/status`, 'POST', { status }),
  listSubjects: (examId: number) => get<ExamSubject[]>(`${BASE}/examinations/${examId}/subjects`),
  assignSubject: (examId: number, payload: ExamSubjectCreate) => send<ExamSubject>(`${BASE}/examinations/${examId}/subjects`, 'POST', payload),
  updateSubject: (examId: number, assignmentId: number, payload: ExamSubjectCreate) => send<ExamSubject>(`${BASE}/examinations/${examId}/subjects/${assignmentId}`, 'PATCH', payload),
  enterScores: (examId: number, entries: ScoreEntry[]) => send<{ created: number; updated: number }>(`${BASE}/examinations/${examId}/entries`, 'POST', { entries }),
  listEntries: (examId: number, subjectId?: number, studentId?: number) => { const q = new URLSearchParams(); if (subjectId != null) q.set('subject_id', String(subjectId)); if (studentId != null) q.set('student_id', String(studentId)); const s = q.toString(); return get<ExamEntry[]>(`${BASE}/examinations/${examId}/entries${s ? `?${s}` : ''}`) },
  generateResults: (examId: number, context: AcademicContext = {}) => { const q = contextQuery(context); return get<StudentResult[]>(`${BASE}/examinations/${examId}/results${q ? `?${q}` : ''}`) },
  resultsAnalysis: (examId: number) => get<ResultsAnalysis>(`${BASE}/examinations/${examId}/results/analysis`),
  listGradeScale: () => get<GradeScale[]>(`${BASE}/examinations/grade-scale`),
  createGradeScale: (payload: GradeScaleCreate) => send<GradeScale>(`${BASE}/examinations/grade-scale`, 'POST', payload),
}

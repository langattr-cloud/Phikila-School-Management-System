...
async function fetchWithFallback(url: string, fallbackUrls: Array<string | null>, init: RequestInit): Promise<Response> {
  const candidates = [url, ...fallbackUrls].filter((candidate, index, all): candidate is string => Boolean(candidate) && all.indexOf(candidate) === index); let lastError: unknown = null
  for (const candidate of candidates) { try { const response = await fetch(candidate, init); if (response.status < 500 || candidate === candidates[candidates.length - 1]) return response } catch (error) { lastError = error; if (candidate === candidates[candidates.length - 1]) throw error } }
  if (lastError instanceof Error) throw lastError; throw new Error('Network request failed')
}
export async function apiFetch<T>(path: string, init: RequestInit = {}, authenticated = true): Promise<T> {
  ...
  const request = async (token: string | null) => { const headers = new Headers(baseHeaders); if (authenticated && token) headers.set('Authorization', `Bearer ${token}`); const primaryUrl = requestUrl(apiUrl, path); const fallbackUrls = [productionFallbackApiUrl ? requestUrl(productionFallbackApiUrl, path) : null]; let response: Response; try { response = await fetchWithFallback(primaryUrl, fallbackUrls, { ...init, headers }) } catch (error) { throw new ApiError(`API could not be reached: ${error instanceof Error ? error.message : 'Network request failed'}`, 0) } if (!response.ok) { const payload = await response.json().catch(() => null); const raw = payload?.detail; const message = typeof raw === 'string' ? raw : typeof raw?.message === 'string' ? raw.message : `Request failed (${response.status})`; throw new ApiError(message, response.status, typeof raw === 'object' ? raw : undefined) } if (response.status === 204) return undefined as T; return response.json() as Promise<T> }
  let token = authenticated ? await currentAccessToken() : null; if (authenticated && !token) throw new ApiError('Please sign in again.', 401); try { return await request(token) } catch (error) { if (authenticated && error instanceof ApiError && error.status === 401 && supabase) { token = await refreshSessionToken(); if (token) return await request(token) } throw error }
}
...
export const api = {
  health: () => apiFetch<{ status: string; environment: string }>('/health', {}, false), me: () => apiFetch<Identity>('/api/v1/auth/me'), school: () => apiFetch<SchoolProfile>('/api/v1/school/'), academicYears: () => apiFetch<AcademicYear[]>('/api/v1/academics/years'),
  createAcademicYear: (payload: { name: string; start_date: string; end_date: string; is_current?: boolean; status?: string }) => apiFetch<AcademicYear>('/api/v1/academics/years', { method: 'POST', body: JSON.stringify(payload) }), updateAcademicYear: (id: number, payload: Partial<Pick<AcademicYear, 'name' | 'start_date' | 'end_date' | 'is_current' | 'status'>>) => apiFetch<AcademicYear>(`/api/v1/academics/years/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  terms: () => apiFetch<Term[]>('/api/v1/academics/terms'), createTerm: (payload: { name: string; start_date?: string | null; end_date?: string | null; is_current?: boolean; academic_year_id: number }) => apiFetch<Term>('/api/v1/academics/terms', { method: 'POST', body: JSON.stringify(payload) }), updateTerm: (id: number, payload: Partial<Pick<Term, 'name' | 'start_date' | 'end_date' | 'is_current' | 'academic_year_id'>>) => apiFetch<Term>(`/api/v1/academics/terms/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  levels: () => apiFetch<Level[]>('/api/v1/academics/levels'), createLevel: (payload: { name: string; code: string; display_order: number; status?: boolean }) => apiFetch<Level>('/api/v1/academics/levels', { method: 'POST', body: JSON.stringify(payload) }), updateLevel: (id: number, payload: Partial<Pick<Level, 'name' | 'code' | 'display_order' | 'status'>>) => apiFetch<Level>(`/api/v1/academics/levels/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  grades: (levelId?: number) => apiFetch<Grade[]>(`/api/v1/academics/grades${levelId != null ? `?level_id=${levelId}` : ''}`), createGrade: (payload: { level_id: number; name: string; code: string; display_order: number; status?: boolean }) => apiFetch<Grade>('/api/v1/academics/grades', { method: 'POST', body: JSON.stringify(payload) }), updateGrade: (id: number, payload: Partial<Pick<Grade, 'name' | 'code' | 'display_order' | 'status'>>) => apiFetch<Grade>(`/api/v1/academics/grades/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  streams: (academicYearId: number, gradeId: number) => apiFetch<Stream[]>(`/api/v1/academics/years/${academicYearId}/grades/${gradeId}/streams`), createStream: (payload: { academic_year_id: number; level_id: number; grade_id: number; name: string; code?: string | null; capacity?: number | null; status?: StreamStatus }) => apiFetch<Stream>('/api/v1/academics/streams', { method: 'POST', body: JSON.stringify(payload) }), updateStream: (streamId: number, payload: Partial<Pick<Stream, 'name' | 'code' | 'capacity' | 'status' | 'class_teacher_id'>>) => apiFetch<Stream>(`/api/v1/academics/streams/${streamId}`, { method: 'PATCH', body: JSON.stringify(payload }),

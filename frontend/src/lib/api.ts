...
async function fetchWithFallback(url: string, fallbackUrl: string | null, init: RequestInit): Promise<Response> { try { return await fetch(url, init) } catch (error) { if (fallbackUrl && fallbackUrl !== url) return fetch(fallbackUrl, init); throw error } }
...
export const api = {
  health: () => apiFetch<{ status: string; environment: string }>('/health', {}, false), me: () => apiFetch<Identity>('/api/v1/auth/me'), school: () => apiFetch<SchoolProfile>('/api/v1/school/'),
  academicYears: () => apiFetch<AcademicYear[]>('/api/v1/academics/years'),
  createAcademicYear: (payload: { name: string; start_date: string; end_date: string; is_current?: boolean; status?: string }) => apiFetch<AcademicYear>('/api/v1/academics/years', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  levels: () => apiFetch<Level[]>('/api/v1/academics/levels'), updateLevel: (id: number, payload: Partial<Pick<Level, 'name' | 'code' | 'display_order' | 'status'>>) => apiFetch<Level>(`/api/v1/academics/levels/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),

...
export const api = {
  ...
  createGrade: (payload: { level_id: number; name: string; code: string; display_order: number; status?: boolean }) => apiFetch<Grade>('/api/v1/academics/grades', { method: 'POST', body: JSON.stringify(payload) }),
  updateGrade: (id: number, payload: Partial<Pick<Grade, 'name' | 'code' | 'display_order' | 'status'>>) => apiFetch<Grade>(`/api/v1/academics/grades/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),

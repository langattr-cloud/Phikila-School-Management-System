...
function TimetableAppearanceLauncher() { const { pathname } = useRouter(); const route = normalisePath(pathname); const isTimetable = route === '/timetable' || route === '/my-timetable'; const [open, setOpen] = useState(false); const [selectedCell, setSelectedCell] = useState<SelectedCell | null>(null); useEffect(() => { if (!isTimetable) return; const handle = (event: Event) => { const detail = (event as CustomEvent<SelectedCell>).detail; if (!detail) return; setSelectedCell(detail); setOpen(true) }; window.addEventListener('phikila:timetable-cell-selected', handle); return () => window.removeEventListener('phikila:timetable-cell-selected', handle) }, [isTimetable]); if (!isTimetable) return null; return <><button type="button" className="timetable-appearance-launcher button button--secondary button--sm" onClick={() => setOpen(true)} aria-label="Open timetable appearance settings">Appearance</button><TimetableAppearanceEditor open={open} onClose={() => setOpen(false)} selectedCell={selectedCell} /></> }

...
export function RequirementsPage() {
  ...
  const [form, setForm] = useState({ class_id: '', subject_id: '', teacher_id: '', room_id: '', periods_per_week: 4, double_periods: 0 })
  ...
  const selectedName = useMemo(() => {
    ...
    return viewMode === 'class' ? data.classes.find((item) => String(item.id) === selectedId)?.name ?? '' : data.teachers.find((item) => String(item.id) === selectedId)?.name ?? ''
  }, [data, selectedId, viewMode])
  ...
  async function add(event: FormEvent) {
    ...
    setSaving(true)
    try {
      await scheduling.createRequirement({ class_id: Number(form.class_id), subject_id: Number(form.subject_id), teacher_id: Number(form.teacher_id), room_id: form.room_id ? Number(form.room_id) : null, periods_per_week: Number(form.periods_per_week), double_periods: Number(form.double_periods) })
      notify('Teaching allocation added.', 'success'); setForm((current) => ({ ...current, subject_id: '', double_periods: 0 })); await load()
  ...
  return <>
    <PageHeader title="Teaching allocations" description="Define the lesson cards the generator must place: class, subject, teacher, room, weekly frequency and double lessons." breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Scheduling' }, { label: 'Teaching Allocations' }]} />
    {error ? <ErrorState title="Teaching allocations could not load" message={error} onRetry={load} /> : <>
      ...
      {ready && data && <>
        <section className="card section">
          <div className="toolbar"><div><h2 className="section__title">Allocate by</h2><p className="section__description">Build requirements by class, or inspect a teacher's workload.</p></div><div className="form__row"><button type="button" className={`button button--sm ${viewMode === 'class' ? 'button--primary' : 'button--ghost'}`} onClick={() => { setViewMode('class'); setSelectedId(data.classes[0] ? String(data.classes[0].id) : '') }}>Grade / Class</button><button type="button" className={`button button--sm ${viewMode === 'teacher' ? 'button--primary' : 'button--ghost'}`} onClick={() => { setViewMode('teacher'); setSelectedId(data.teachers[0] ? String(data.teachers[0].id) : '') }}>Teacher</button></div></div>
          ...
          <div className="panel__subtitle">{viewMode === 'class' ? 'Subjects and teachers for' : 'Current workload for'} {selectedName}</div><div className="chip-list"><Badge>{totalWorkload} lessons / week</Badge>{viewMode === 'class' && <Badge>Generator-ready lesson cards</Badge>}</div>
        </section>
        <section className="card section"><h2 className="section__title">Add lesson requirement</h2><p className="section__description">A requirement is the unit the timetable generator must place. Double lessons are kept consecutive.</p><form className="form form--grid" onSubmit={add}>
          ...
          <div className="form__row form--grid__full"><button className="button button--primary" type="submit" disabled={saving}>{saving ? 'Saving…' : 'Add lesson requirement'}</button></div>
        </form></section>
      </>}
      <section className="card section"><div className="toolbar"><div className="search"><SearchIcon className="search__icon" width={18} height={18} /><label className="visually-hidden" htmlFor="req-search">Search allocations</label><input id="req-search" className="input input--search" type="search" placeholder="Search subject, class or teacher" value={query} onChange={(event) => setQuery(event.target.value)} /></div>{query && <button type="button" className="button button--ghost button--sm" onClick={() => setQuery('')}>Clear search</button>}</div><DataTable caption="Teaching allocations" columns={columns} rows={filtered} rowKey={(row) => row.id} loading={loading} loadingLabel="Loading teaching allocations" empty={<EmptyState title={query ? 'No matching allocations' : `No allocations for ${selectedName || 'this selection'}`} description={query ? 'Nothing matches your search.' : 'Allocate a subject with a teacher and lessons per week above.'} icon={<CalendarIcon width={22} height={22} />} />} rowActions={(row) => <button type="button" className="button button--ghost button--sm" onClick={() => remove(row)}>Delete</button>} /></section>
    </>}
  </>
}

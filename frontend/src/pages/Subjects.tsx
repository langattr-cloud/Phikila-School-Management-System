...
import { scheduling, type Subject, type SubjectInput } from '../lib/scheduling'
import { SubjectColourManager } from '../components/SubjectColourManager'

type SubjectFormProps = { initial: Subject | null; onCancel: () => void; onSaved: () => void }

function SubjectForm({ initial, onCancel, onSaved }: SubjectFormProps) {
  const { notify } = useToast()
  const [values, setValues] = useState({ name: initial?.name ?? '', code: initial?.code ?? '', colour: initial?.colour ?? '#0F2A47' })

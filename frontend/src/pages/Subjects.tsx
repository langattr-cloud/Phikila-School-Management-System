...
export default function Subjects() {
  ...
  const columns = useMemo<Column<Subject>[]>(() => [{ key: 'name', header: 'Name', render: (row) => <div className="subject-name-cell"><span className="subject-color" style={{ display: 'inline-block', width: 16, height: 16, minWidth: 16, borderRadius: 4, backgroundColor: row.colour || '#0F2A47', border: '1px solid rgba(15, 42, 71, 0.18)' }} aria-label={`Subject color ${row.colour || '#0F2A47'}`} /> <span>{row.name}</span></div> }, { key: 'code', header: 'Code', render: (row) => row.code }, { key: 'color', header: 'Color', render: (row) => <div className="subject-color-cell"><span className="subject-color" style={{ display: 'inline-block', width: 28, height: 28, minWidth: 28, borderRadius: 6, backgroundColor: row.colour || '#0F2A47', border: '1px solid rgba(15, 42, 71, 0.18)' }} aria-label={`Subject color ${row.colour || '#0F2A47'}`} /><span>{row.colour || '—'}</span></div> }], [])

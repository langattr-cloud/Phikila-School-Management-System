...
const FEATURES = [
  { icon: '◈', title: 'School Profile', desc: 'Keep your school identity, details, contacts, and configuration in one secure place.' },
  { icon: '◌', title: 'Academic Calendar', desc: 'Manage academic years, terms, levels, and the structure behind your school day.' },
  { icon: '⌘', title: 'Teachers & Subjects', desc: 'Connect your teaching team, subjects, classes, and curriculum structure.' },
  { icon: '◇', title: 'Students', desc: 'Manage enrolment, class assignments, attendance, academics, and student records.' },
  { icon: '▦', title: 'Timetable', desc: 'Build and manage schedules with rooms, teachers, constraints, and conflict visibility.' },
  { icon: '✦', title: 'Intelligence', desc: 'Use Phikila Copilot to understand scheduling operations and surface what needs attention.' },
]
  ...
  return <div className="landing">
    <header className="landing__hero">
      ...
      <div className="landing__hero-content">
        <p className="eyebrow">✦ Aurora intelligence · School operations</p>
    ...
    <section className="landing__features"><div className="landing__features-inner"><h2 className="landing__section-title">One system. Every operation.</h2><p className="landing__section-subtitle">A connected workspace designed for the people who run schools.</p><div className="landing__feature-grid">{FEATURES.map((feature) => <article className="landing__feature-card" key={feature.title}><span className="landing__feature-icon" aria-hidden="true">{feature.icon}</span><h3 className="landing__feature-title">{feature.title}</h3><p className="landing__feature-desc">{feature.desc}</p></article>)}</div></div></section>

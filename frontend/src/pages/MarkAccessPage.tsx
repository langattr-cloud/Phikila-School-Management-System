...
export default function MarkAccessPage() {
  ...
  useEffect(() => {
    void Promise.all([scheduling.me(), examinations.list(), api.academicYears(), api.levels(), scheduling.teachers(), scheduling.subjects()])
      .then(([me, nextExams, nextYears, nextLevels, nextTeachers, nextSubjects]) => {
        setPrincipal(me); setExams(nextExams); setYears(nextYears); setLevels(nextLevels); setTeachers(nextTeachers)
  ...
  async function assignTeacher() {
    ...
    try {
      ...
      const payload = { subject_id: Number(subjectId), academic_year_id: Number(yearId), level_id: Number(levelId), grade_id: Number(gradeId), stream_id: Number(streamId), teacher_id: assignmentTeacherId ? Number(assignmentTeacherId) : null, total_marks: existing?.total_marks ?? selectedExam.total_marks }
      ...
      setMessage('Teacher assignment saved. The assigned teacher can upload marks until the examination is analyzed.')
  ...
  async function saveMarks() {
    ...
    setBusy(true); setMessage(null)
    try {
      const payload = students.filter(student => entries[student.id]?.score !== '').map(student => ({ student_id: student.id, subject_id: selectedAssignment.subject_id, score: Number(entries[student.id]?.score), remarks: entries[student.id]?.remarks || null }))
  ...
  return <div>
    <PageHeader title="Marks & Results" description="Select Level → Grade → Stream → Subject. Before analysis, only DOS and explicitly assigned teachers can work with marks. After analysis, every teacher can view the full examination." breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Examinations', to: '/examinations' }, { label: 'Marks & Results' }]} />
    {error && <Alert tone="error">{error}</Alert>}
    {message && <Alert tone="info">{message}</Alert>}

    <section className="card section">
      <h2 className="section__title">Academic context</h2>
    ...
    {isDos && !examIsAnalyzed && selectedExam && levelId && gradeId && streamId && subjectId && <section className="card section">
      <h2 className="section__title">DOS teacher permission</h2>
      <p style={{ color: 'var(--color-ink-muted)', fontSize: '.85rem' }}>Assign a teacher to this exact Level, Grade, Stream and Subject. This assignment grants mark-upload access until the examination is analyzed.</p>
      <div className="form form--grid"><Select label="Teacher" value={assignmentTeacherId} options={teachers.map(item => [item.id, item.name])} onChange={setAssignmentTeacherId} /></div>
      <button className="button button--primary" disabled={busy} onClick={assignTeacher}>{busy ? 'Saving…' : 'Grant / Update Upload Permission'}</button>
    </section>}

    {examIsAnalyzed && principal?.role === 'teacher' && <Alert tone="info">This examination has been analyzed. You now have view-only access to all grades, streams, subjects, results and rankings. Mark editing is disabled.</Alert>}

    {streamId && subjectId && <section className="card section">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div><h2 className="section__title">{canUpload ? 'Upload marks' : 'View marks'}</h2><p style={{ color: 'var(--color-ink-muted)', margin: 0 }}>{subjects.find(item => item.id === Number(subjectId))?.name ?? 'Subject'} · {streams.find(item => item.id === Number(streamId))?.name ?? ''}</p></div>
        {selectedAssignment && <Badge tone={canUpload ? 'success' : 'info'}>{canUpload ? 'Upload permission granted' : 'View only'}</Badge>}
      </div>
      {!selectedAssignment ? <EmptyState title="No subject assignment" description={examIsAnalyzed ? 'No results are available for this subject in the selected examination.' : 'DOS must assign a teacher to this subject and stream before marks can be uploaded.'} /> : <>
        <div style={{ overflowX: 'auto', marginTop: 'var(--space-3)' }}><table style={{ width: '100%' }}><thead><tr><th>Learner</th><th>Admission No.</th><th>Score / {selectedAssignment.total_marks}</th><th>Remarks</th></tr></thead><tbody>

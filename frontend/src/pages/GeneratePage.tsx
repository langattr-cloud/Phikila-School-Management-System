    ...
    <div className="conflict-list" role="alert">
      ...
      <ol className="check-list">
        {conflicts.map((conflict, index) => {
          ...
          return <li key={`${conflict.kind}-${index}`} className="check-row check-row--failed" style={{ display: 'block', padding: '12px 0' }}><strong>{conflict.kind.replaceAll('_', ' ')}</strong><div>{conflict.message}</div>{(conflict.day != null || conflict.period != null) && <div className="form__note">Location: {dayName(conflict.day)}{slot ? ` · ${slot.name} (${slot.start_time}–${slot.end_time})` : conflict.period != null ? ` · Period ${conflict.period}` : ''}</div>}{lessonsText.length > 0 && <div className="form__note">Lessons: {lessonsText.join(' | ')}</div>}</li>
        })}
      </ol>
    </div>
  )
...
function LessonPreview({ calendar, lessons, subjects, classes }: { calendar: Calendar | null; lessons: Lesson[]; subjects: Subject[]; classes: SchoolClass[] }) {
  ...
  return <section className="card section lesson-preview">
    <div className="panel__head"><div><h2 className="section__title">Lesson preview</h2><p className="form__note">Established working days and periods with the accumulated lessons visible before generation.</p></div><Badge tone="neutral">{lessons.length} lessons</Badge></div>
    <div className="lesson-preview__subjects" aria-label="Subjects">{subjects.map((subject) => <span key={subject.id} className="lesson-preview__subject"><span className="lesson-preview__swatch" style={{ backgroundColor: subject.color || '#64748b' }} aria-hidden="true" />{subject.name}</span>)}</div>
    <div className="lesson-preview__grid-wrap"><table className="lesson-preview__grid"><thead><tr><th>Period</th>{activeDaysList.map((day) => <th key={day.index}>{day.name}</th>)}</tr></thead><tbody>{teachingPeriods.map((period) => <tr key={period.index}><th>{period.name}<span>{period.start_time}–{period.end_time}</span></th>{activeDaysList.map((day) => { const lesson = lessonAt(day.index, period.index); const subject = lesson ? subjectMap.get(lesson.subject_id) : null; const schoolClass = lesson ? classMap.get(lesson.class_id) : null; return <td key={`${day.index}-${period.index}`}>{lesson && subject ? <div className="lesson-preview__card" style={{ borderLeftColor: subject.color || '#64748b' }}><strong>{subject.name}</strong>{schoolClass && <span>{schoolClass.name}</span>}</div> : <span className="lesson-preview__empty">—</span>}</td> })}</tr>)}</tbody></table></div>
    <div className="lesson-preview__grades"><h3 className="panel__subtitle">Grades / classes</h3><div className="lesson-preview__grade-list">{classes.map((schoolClass) => <span key={schoolClass.id} className="badge">{schoolClass.name}</span>)}</div></div>
  </section>
}
...
export function GeneratePage() {
  ...
  const loadSummary = useCallback(async () => {
    setLoading(true)
    try {
      const [dashboard, currentVersion, activeJob, calendar, subjects, classes] = await Promise.all([scheduling.dashboard(), scheduling.currentVersion(), scheduling.activeJob(), scheduling.calendar(), scheduling.subjects(), scheduling.classes()])
      const lessons = currentVersion ? await scheduling.lessons(currentVersion.id) : []
      setSummary(dashboard); setHasTimetable(currentVersion !== null); setPreview({ calendar, lessons, subjects, classes })
      if (activeJob) rememberJob(activeJob)
      else if (typeof window !== 'undefined') { const storedId = Number(window.localStorage.getItem(JOB_STORAGE_KEY)); if (Number.isInteger(storedId) && storedId > 0) { try { rememberJob(await scheduling.job(storedId)) } catch { window.localStorage.removeItem(JOB_STORAGE_KEY); rememberJob(null) } } else rememberJob(null) } else rememberJob(null)
      setError(null)
    } catch (err) { setError(friendlyApiError(err, 'load your school setup')) } finally { setLoading(false) }
  }, [rememberJob])

  useEffect(() => { void loadSummary() }, [loadSummary])
  const loadConflicts = useCallback(async (versionId: number) => { try { const [found, calendar, lessons, teachers, subjects, classes, rooms] = await Promise.all([scheduling.conflicts(versionId), scheduling.calendar(), scheduling.lessons(versionId), scheduling.teachers(), scheduling.subjects(), scheduling.classes(), scheduling.rooms()]); setConflicts(found.filter((conflict) => conflict.severity === 'hard')); setConflictContext({ calendar, lessons, teachers, subjects, classes, rooms }) } catch { setConflicts([]) } }, [])
  useEffect(() => { if (job?.status === 'failed' && job.result_version_id) void loadConflicts(job.result_version_id); else if (job?.status !== 'failed') setConflicts([]) }, [job?.status, job?.result_version_id, loadConflicts])
  useEffect(() => {
    if (!job || !ACTIVE.has(job.status)) return
    let cancelled = false
    const poll = async () => { try { const next = await scheduling.job(job.id); if (cancelled) return; rememberJob(next); if (!ACTIVE.has(next.status)) { if (next.status === 'completed') { setHasTimetable(true); notify('Timetable generated. Use View timetable to open it.', 'success'); if (typeof window !== 'undefined') window.localStorage.removeItem(JOB_STORAGE_KEY) } else if (next.status === 'failed') { notify(next.message || 'Generation could not finish.', 'error'); if (typeof window !== 'undefined') window.localStorage.removeItem(JOB_STORAGE_KEY) } else if (next.status === 'cancelled') { notify('Timetable generation was cancelled.', 'info'); if (typeof window !== 'undefined') window.localStorage.removeItem(JOB_STORAGE_KEY) } return }; timer.current = window.setTimeout(poll, 900) } catch { if (!cancelled) timer.current = window.setTimeout(poll, 2000) } }

...
export function GeneratePage(){
 ...
 async function load(){
  ...
  try{
   ...
   const first=tt.find(t=>t.is_active)??tt[0]??null
   setLabels(Object.fromEntries(cal.days.map(d=>[d.index,first?.display_mode==='date'?(d.date_value??''):d.name])))
   if(first){
    setTypeId(first.id); setMode(first.display_mode); setDays(first.day_indexes)
    setPeriods(cal.periods.filter(p=>p.is_teaching).map(p=>p.index))
   } else {
    setTypeId(null); setDays([]); setPeriods([]); setMode(cal.display_mode??'day')
   }
 ...
 function selectType(id:number){
  ...
  setTypeId(id); setMode(t.display_mode); setDays(t.day_indexes)
  setPeriods(teachingPeriods.map(p=>p.index))
  setLabels(Object.fromEntries(configuredDays.map(d=>[d.index,t.display_mode==='date'?(d.date_value??''):d.name])))
 }

 function beginNew(){
  setName(''); setCode(''); setMode('day')
  setTypeDays(configuredDays.map(d=>d.index)); setEditor(true)
 }

 function beginEdit(){
  if(!selectedType)return
  setName(selectedType.name); setCode(selectedType.code)
  setMode(selectedType.display_mode); setTypeDays(selectedType.day_indexes); setEditor(true)
 }

 async function saveType(){
  if(!name.trim()||!code.trim()||!typeDays.length)return
  try{
   const payload={name:name.trim(),code:code.trim(),display_mode:mode,day_indexes:typeDays,is_active:true,is_system:false}
   const saved=selectedType
    ? await scheduling.updateTimetableType(selectedType.id,payload)
    : await scheduling.createTimetableType(payload)
   setTypes(v=>selectedType?v.map(t=>t.id===saved.id?saved:t):[...v,saved])
   setTypeId(saved.id); setMode(saved.display_mode); setDays(saved.day_indexes)
   setEditor(false)
   notify(selectedType?'Timetable type updated.':'Timetable type created.','success')
 ...
 async function generate(){
  ...
  if(selectedType?.display_mode==='date'&&days.some(i=>!(labels[i]??'').trim())){
   notify('Enter a display date for every selected day.','error'); return
  }
  setStarting(true)
  try{
   const next=await scheduling.generateProfile({
    ...
    day_indexes:days,
    day_names:Object.fromEntries(days.map(i=>[i,(labels[i]??configuredDays.find(d=>d.index===i)?.name??String(i)).trim()])),
    max_seconds:30,
   })
 ...
 useEffect(()=>{
  ...
  const timer=window.setInterval(()=>void scheduling.job(job.id).then(setJob).catch(()=>undefined),1000)
 ...
 return <>
  <PageHeader title="Generate timetable" description="Select a configured timetable type, schedule days and teaching periods, then generate a draft." breadcrumbs={[{label:'Dashboard',to:'/'},{label:'Timetable',to:'/timetable'},{label:'Generate'}]}/>
  {job&&<section className="card section"><div className="panel__head"><div><h2 className="section__title">Generation {running?'in progress':'result'}</h2><p className="form__note">{job.message||job.stage||'Solver job'}</p></div><Badge tone={job.status==='completed'?'success':job.status==='failed'?'danger':'neutral'}>{job.status}</Badge></div><div className="progress" role="progressbar" aria-valuenow={job.progress} aria-valuemin={0} aria-valuemax={100}><div className="progress__bar" style={{width:`${Math.max(0,Math.min(100,job.progress))}%`}}/></div>{job.result_version_id&&job.status==='completed'&&<p className="form__note" style={{marginTop:8}}>Draft version #{job.result_version_id} is ready for review.</p>}</section>}
  <section className="card section"><div className="panel__head"><div><p className="form__note">TIMETABLE CONFIGURATION</p><h2 className="section__title">Build a timetable</h2><p className="form__note">Configuration is read from the school's scheduling model. Generation uses the existing solver API.</p></div><Badge tone="neutral">Draft</Badge></div>
   <div className="form form--grid">
    ...
    {selectedType&&<div className="field form--grid__full"><label className="field__label">Schedule {selectedType.display_mode==='date'?'dates':'days'}</label><div className="chip-toggles">{configuredDays.filter(d=>selectedType.day_indexes.includes(d.index)).map(d=><label key={d.index} className={`chip-toggle ${days.includes(d.index)?'chip-toggle--on':''}`}><input type="checkbox" checked={days.includes(d.index)} onChange={()=>toggle(days,d.index,setDays)}/><span>{labels[d.index]||d.name}</span></label>)}</div></div>}
    {selectedType&&<div className="field form--grid__full"><label className="field__label">{selectedType.display_mode==='date'?'Display dates':'Display labels'}</label><div className="form form--grid">{configuredDays.filter(d=>selectedType.day_indexes.includes(d.index)).map(d=><div className="field" key={d.index}><label className="field__label" htmlFor={`schedule-label-${d.index}`}>{d.name}</label><input id={`schedule-label-${d.index}`} type={selectedType.display_mode==='date'?'date':'text'} className="input" value={labels[d.index]??(selectedType.display_mode==='date'?(d.date_value??''):d.name)} onChange={e=>setLabels(v=>({...v,[d.index]:e.target.value}))}/></div>)}</div></div>}

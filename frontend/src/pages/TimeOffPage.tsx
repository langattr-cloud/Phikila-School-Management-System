...
export function TimeOffPage() {
  ...
  useEffect(() => {
    ...
    const next = selected.unavailable ?? EMPTY
    setDraft(next)
    setSaved(next)
  }, [selected])
  ...
  async function save() {
    ...
    setError(null)
    try {
      const updated = kind === 'teachers'
        ? await scheduling.updateTeacher(selected.id, { unavailable: draft })
        : await scheduling.updateClass(selected.id, { unavailable: draft })
  ...
  return <>
    <PageHeader
      title="Time off"
      description="Mark when teachers or classes are unavailable for lessons. Click any cell to switch between available and time off."

...
export function TimeOffPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([scheduling.calendar(), scheduling.teachers(), scheduling.classes()])
      .then(([nextCalendar, nextTeachers, nextClasses]) => {
        if (!active) return
        setCalendar(nextCalendar); setTeachers(nextTeachers); setClasses(nextClasses)
        setSelectedId(nextTeachers[0]?.id ?? nextClasses[0]?.id ?? null)
      })
  ...
  async function save() {
    ...
    try {
      const updated = kind === 'teachers' ? await scheduling.updateTeacher(selected.id, { unavailable: draft }) : await scheduling.updateClass(selected.id, { unavailable: draft })
      if (kind === 'teachers') setTeachers((current) => current.map((item) => item.id === updated.id ? updated as Teacher : item))
      else setClasses((current) => current.map((item) => item.id === updated.id ? updated as SchoolClass : item))
  ...
  return <>
    <PageHeader title="Time off" description="Mark when teachers or classes are unavailable for lessons. Click any cell to switch between available and time off." breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Scheduling' }, { label: 'Time off' }]} actions={<button type="button" className="button button--primary button--sm" onClick={save} disabled={!dirty || saving || !selected}>{saving ? 'Saving…' : 'Save changes'}</button>} />

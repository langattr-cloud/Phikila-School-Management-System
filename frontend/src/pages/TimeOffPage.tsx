...
export function TimeOffPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    Promise.all([scheduling.calendar(), scheduling.teachers(), scheduling.classes(), scheduling.subjects(), scheduling.constraints()])
  ...
  if (loading) {
    return <><PageHeader title="Time off" description="Mark unavailable lesson periods using the timetable grid." breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Scheduling' }, { label: 'Time off' }]} /><section className="card section"><p className="form__note">Loading days, periods and resources…</p></section></>
  }
  ...
  return <>
    ...
    <section className="card section">
      ...
      {!selected ? <div><p className="form__note">Add a {kind === 'teachers' ? 'teacher' : kind === 'classes' ? 'class' : 'subject'} first.</p><Link className="button button--secondary button--sm" to={kind === 'teachers' ? '/setup/teachers' : kind === 'classes' ? '/setup/classes' : '/setup/subjects'}>Configure {kind === 'teachers' ? 'teachers' : kind === 'classes' ? 'classes' : 'subjects'}</Link></div> : calendar && days.length && periods.length ? <>
        <div className="timeoff-toolbar"><div><h2 className="section__title" style={{ marginBottom: '0.15rem' }}>{selected.name}</h2><p className="field__hint">Click any period. <strong>✓</strong> means available; <strong>X</strong> means time off.</p></div><div className="timeoff-legend" aria-label="Time-off legend"><span className="timeoff-legend__item timeoff-legend__item--available"><strong>✓</strong> Available</span><span className="timeoff-legend__item timeoff-legend__item--blocked"><strong>X</strong> Time off</span><span className="timeoff-legend__count">{blockedCount} blocked</span></div></div>
        <div className="timeoff-grid-wrap"><table className="timeoff-grid"><thead><tr><th scope="col" className="timeoff-grid__corner">Day</th>{periods.map((period, index) => <th key={period.index} scope="col" className="timeoff-grid__period" title={`${period.start_time}–${period.end_time}`}><span className="timeoff-grid__period-number">{index + 1}</span><span className="timeoff-grid__period-name">{period.name}</span><span className="timeoff-grid__period-time">{period.start_time}–{period.end_time}</span></th>)}</tr></thead><tbody>{days.map((day) => <tr key={day.index}><th scope="row" className="timeoff-grid__day">{day.name}</th>{periods.map((period) => { const unavailable = (draft[String(day.index)] ?? []).includes(period.index); return <td key={period.index} className="timeoff-grid__cell"><button type="button" onClick={() => toggle(day.index, period.index)} aria-pressed={unavailable} aria-label={`${day.name}, ${period.name}: ${unavailable ? 'time off' : 'available'}`} className={`timeoff-grid__button${unavailable ? ' timeoff-grid__button--blocked' : ''}`}><span aria-hidden="true" className="timeoff-grid__mark">{unavailable ? 'X' : '✓'}</span></button></td> })}</tr>)}</tbody></table></div>
    ...
    <style>{`\n      .timeoff-toolbar{display:flex;align-items:flex-end;justify-content:space-between;gap:1rem;margin-bottom:.85rem;flex-wrap:wrap}\n      .timeoff-legend{display:flex;align-items:center;gap:.8rem;flex-wrap:wrap;font-size:.78rem;font-weight:700}\n      .timeoff-legend__item{display:inline-flex;align-items:center;gap:.3rem}.timeoff-legend__item strong{font-size:1rem}.timeoff-legend__item--available{color:var(--color-success)}.timeoff-legend__item--blocked{color:var(--color-danger)}.timeoff-legend__count{color:var(--color-muted)}\n      .timeoff-grid-wrap{overflow-x:auto;border:1px solid var(--color-line);border-radius:var(--radius-md);background:var(--color-surface)}\n      .timeoff-grid{width:100%;min-width:760px;border-collapse:separate;border-spacing:0;table-layout:fixed}\n      .timeoff-grid th,.timeoff-grid td{border-right:1px solid var(--color-line);border-bottom:1px solid var(--color-line)}.timeoff-grid tr>*:last-child{border-right:0}.timeoff-grid tbody tr:last-child>*{border-bottom:0}\n      .timeoff-grid__corner,.timeoff-grid__day{width:7.5rem;background:var(--color-surface-muted);text-align:left}.timeoff-grid__corner{padding:.65rem .7rem;font-size:.78rem;font-weight:800}.timeoff-grid__day{padding:.65rem .7rem;font-size:.82rem;font-weight:800;white-space:nowrap}\n      .timeoff-grid__period{padding:.45rem .25rem .5rem;background:var(--color-surface-muted);text-align:center;vertical-align:middle}.timeoff-grid__period-number{display:block;font-size:.95rem;font-weight:850;line-height:1.1}.timeoff-grid__period-name{display:block;margin-top:.15rem;font-size:.68rem;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.timeoff-grid__period-time{display:block;margin-top:.1rem;font-size:.6rem;color:var(--color-muted);white-space:nowrap}\n      .timeoff-grid__cell{padding:0;height:3.65rem}.timeoff-grid__button{width:100%;height:100%;min-height:3.65rem;display:flex;align-items:center;justify-content:center;border:0;border-radius:0;background:var(--color-success-soft);color:var(--color-success);cursor:pointer}.timeoff-grid__button:hover{filter:brightness(.96)}.timeoff-grid__button:active{transform:scale(.96)}.timeoff-grid__button:focus-visible{outline:2px solid currentColor;outline-offset:-3px}.timeoff-grid__button--blocked{background:var(--color-danger-soft);color:var(--color-danger)}.timeoff-grid__mark{font-size:1.45rem;font-weight:900;line-height:1}\n      .timeoff-actions{display:flex;align-items:center;gap:.75rem;margin-top:.85rem;flex-wrap:wrap}@media(max-width:720px){.timeoff-grid__corner,.timeoff-grid__day{width:6.5rem}.timeoff-grid__period-time{display:none}}\n    `}</style>
  </>
}

  ...
  useEffect(() => {
    ...
    Promise.all([scheduling.calendar(), scheduling.teachers(), scheduling.classes(), scheduling.subjects(), scheduling.constraints()])
      .then(([nextCalendar, nextTeachers, nextClasses, nextSubjects, nextConstraints]) => {
        if (!active) return
        setCalendar(nextCalendar); setTeachers(nextTeachers); setClasses(nextClasses); setSubjects(nextSubjects); setConstraints(nextConstraints)

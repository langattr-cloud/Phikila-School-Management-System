...
export default function ReportCardPage() {
  ...
  useEffect(() => {
    void Promise.all([examinations.list(), api.students(), scheduling.subjects(), api.school()])
      .then(([nextExams, nextStudents, nextSubjects, nextSchool]) => {
        setExams(nextExams); setStudents(nextStudents.items); setSubjects(nextSubjects); setSchool(nextSchool)
  ...
  const visibleStudents = useMemo(() => { const q = studentQuery.trim().toLowerCase(); if (!q) return students.slice(0, 80); return students.filter(item => `${fullName(item)} ${item.admission_number}`.toLowerCase().includes(q)).slice(0, 80) }, [students, studentQuery])

  useEffect(() => {
    if (!selectedExam || !selectedStudent) { setAssignments([]); setEntries([]); return }
    setLoadingCard(true); setError(null)
    void Promise.all([examinations.listSubjects(selectedExam.id), examinations.listEntries(selectedExam.id, undefined, selectedStudent.id)])
      .then(([nextAssignments, nextEntries]) => { setAssignments(nextAssignments); setEntries(nextEntries) })
      .catch(err => setError(friendlyApiError(err, 'load the student report card'))).finally(() => setLoadingCard(false))
  }, [selectedExam, selectedStudent])

  const rows = useMemo<Row[]>(() => assignments.map(assignment => ({ ...assignment, subjectName: subjects.find(subject => subject.id === assignment.subject_id)?.name || `Subject ${assignment.subject_id}`, entry: entries.find(entry => entry.subject_id === assignment.subject_id) })), [assignments, entries, subjects])
  ...
  const markedRows = rows.filter(row => row.entry?.score != null)
  const averagePoints = markedRows.length ? markedRows.reduce((sum, row) => sum + ((row.entry!.score! / row.total_marks) * 8), 0) / markedRows.length : 0
  ...
  return <>
    <div className="no-print">
      ...
      {loading ? <LoadingBlock label="Loading report-card data" rows={4} /> : <section className="card section"><div className="form form--grid"><div className="field"><label className="field__label" htmlFor="report-exam">Examination</label><select id="report-exam" className="input" value={examId} onChange={event => setExamId(event.target.value)}><option value="">Select examination</option>{exams.map(exam => <option key={exam.id} value={exam.id}>{exam.name}{exam.exam_date ? ` · ${exam.exam_date}` : ''}</option>)}</select></div><div className="field"><label className="field__label" htmlFor="student-search">Find student</label><input id="student-search" className="input" value={studentQuery} onChange={event => setStudentQuery(event.target.value)} placeholder="Name or admission number" /></div><div className="field"><label className="field__label" htmlFor="report-student">Student</label><select id="report-student" className="input" value={studentId} onChange={event => setStudentId(event.target.value)}><option value="">Select student</option>{visibleStudents.map(student => <option key={student.id} value={student.id}>{fullName(student)} · {student.admission_number}</option>)}</select></div></div><p style={{ color: 'var(--color-ink-muted)', fontSize: '.85rem', marginBottom: 0 }}>Select an examination and student. The printable report uses the marks already entered for that examination.</p></section>}
    </div>
    {!selectedExam || !selectedStudent ? <div className="no-print"><EmptyState title="Choose an examination and student" description="The completed report card will appear here for review and printing." /></div> : loadingCard ? <LoadingBlock label="Preparing report card" rows={8} /> : <article className="report-card-print">
      ...
      <section className="report-student-grid"><div><span>LEARNER'S NAME</span><strong>{fullName(selectedStudent)}</strong></div><div><span>ADMISSION NO.</span><strong>{selectedStudent.admission_number}</strong></div><div><span>UPI NO.</span><strong>—</strong></div><div><span>STATUS</span><strong>{selectedStudent.status}</strong></div></section>
      <section className="report-summary-grid"><div><span>TOTAL SCORE</span><strong>{formatScore(total)} / {formatScore(maxTotal)}</strong></div><div><span>PERCENTAGE</span><strong>{percentage.toFixed(1)}%</strong></div><div><span>AVERAGE POINTS</span><strong>{Number.isFinite(averagePoints) ? averagePoints.toFixed(2) : '—'}</strong></div><div><span>OVERALL OUTCOME</span><strong>{overallBand}</strong></div></section>

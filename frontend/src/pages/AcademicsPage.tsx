...
export function AcademicsPage() {
 ...
 const [query, setQuery] = useState(''); const [editing,setEditing]=useState<AcademicYear|null>(null); const [name,setName]=useState(''); const [start,setStart]=useState(''); const [end,setEnd]=useState(''); const [current,setCurrent]=useState(false); const [status,setStatus]=useState('ACTIVE'); const [saving,setSaving]=useState(false); const [saveError,setSaveError]=useState<string|null>(null)
 ...
 function openEdit(row:AcademicYear){setEditing(row);setName(row.name);setStart(row.start_date);setEnd(row.end_date);setCurrent(row.is_current===true);setStatus(row.status||'ACTIVE');setSaveError(null)}
 async function save(e:FormEvent){e.preventDefault();if(!editing)return;setSaving(true);setSaveError(null);try{await api.updateAcademicYear(editing.id,{name:name.trim(),start_date:start,end_date:end,is_current:current,status:status.trim()||'ACTIVE'});setEditing(null);await reload()}catch(e){setSaveError(friendlyApiError(e,'save academic year'))}finally{setSaving(false)}}
 const yearColumns: Column<AcademicYear>[] = [
  { key: 'name', header: 'Academic year', render: row => row.name }, { key: 'start', header: 'Starts', render: row => row.start_date }, { key: 'end', header: 'Ends', render: row => row.end_date },
  { key: 'status', header: 'Status', render: row => row.is_current ? <Badge tone="success">Current</Badge> : <Badge>{row.status || 'Recorded'}</Badge> },
  { key: 'actions', header: 'Actions', render: row => <button type="button" className="button button--ghost button--sm" onClick={()=>openEdit(row)}>Edit</button> },
 ]
 const termColumns: Column<Term>[] = [ { key: 'name', header: 'Term', render: row => row.name }, { key: 'start', header: 'Starts', render: row => row.start_date || 'Not set' }, { key: 'end', header: 'Ends', render: row => row.end_date || 'Not set' }, { key: 'status', header: 'Status', render: row => row.is_current ? <Badge tone="success">Current</Badge> : <Badge>Scheduled</Badge> } ]
 return <>
  ...
  {editing&&<div className="streams-modal-backdrop"><div className="card streams-modal" role="dialog" aria-modal="true" aria-labelledby="edit-year-title"><h2 className="section__title" id="edit-year-title">Edit academic year</h2><form onSubmit={save}><label className="label" htmlFor="year-name">Name</label><input id="year-name" className="input" value={name} onChange={e=>setName(e.target.value)} required/><label className="label" htmlFor="year-start">Start date</label><input id="year-start" className="input" type="date" value={start} onChange={e=>setStart(e.target.value)} required/><label className="label" htmlFor="year-end">End date</label><input id="year-end" className="input" type="date" value={end} onChange={e=>setEnd(e.target.value)} required/><label className="label" htmlFor="year-status">Status</label><input id="year-status" className="input" value={status} onChange={e=>setStatus(e.target.value)} required/><label className="label"><input type="checkbox" checked={current} onChange={e=>setCurrent(e.target.checked)}/> Current academic year</label>{saveError&&<p className="error">{saveError}</p>}<div className="streams-modal__actions"><button type="button" className="button button--ghost" onClick={()=>setEditing(null)}>Cancel</button><button type="submit" className="button button--primary" disabled={saving}>{saving?'Saving…':'Save academic year'}</button></div></form></div></div>}
 </>
}

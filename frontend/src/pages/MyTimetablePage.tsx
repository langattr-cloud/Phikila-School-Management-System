...
export function MyTimetablePage() {
  ...
  const myClassIds = useMemo(() => {
    ...
    if (!teacherId) return []
    return [...new Set(requirements.filter(row => row.teacher_id === teacherId).map(row => row.class_id))]
  }, [me, requirements])

  const myClasses = useMemo(() => classes.filter(item => myClassIds.includes(item.id)), [classes, myClassIds])

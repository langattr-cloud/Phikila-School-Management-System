...
import { CalendarDatesPanel } from '../components/CalendarDatesPanel'
...
export function PeriodsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDays(calendar.days.length ? calendar.days.map(({ index, name, is_active }) => ({ index, name, is_active })) : WEEKDAYS.slice(0, 5).map((name, index) => ({ index, name, is_active: true })))
      setPeriods(calendar.periods.length ? calendar.periods.map(({ index, name, start_time, end_time, is_teaching }) => ({ index, name, start_time, end_time, is_teaching })) : DEFAULT_PERIODS.map((period) => ({ ...period })))
    } catch (err) {
  ...
  function toggleDay(index: number) { setDays((current) => current.map((day) => day.index === index ? { ...day, is_active: !day.is_active } : day)) }
  ...
  function updatePeriod(index: number, patch: Partial<Period>) { setPeriods((current) => current.map((period) => period.index === index ? { ...period, ...patch } : period)) }
  function removePeriod(index: number) { setPeriods((current) => current.filter((p) => p.index !== index).map((p, i) => ({ ...p, index: i }))) }
      ...
      {error ? <ErrorState title="School week could not load" message={error} onRetry={load} /> : loading ? (
        ...
        <>
          ...
          <section className="card section">
            ...
            <div className="chip-toggles">{days.map((day) => <label key={day.index} className={`chip-toggle ${day.is_active ? 'chip-toggle--on' : ''}`}><input type="checkbox" checked={day.is_active} onChange={() => toggleDay(day.index)} />{day.name}</label>)}</div>
          </section>
          <section className="card section">
            ...
            {periods.length === 0 ? <><p className="form__note">No periods defined yet.</p><button type="button" className="button button--primary button--sm" onClick={applyPreset}>Use a standard 8-period day</button></> : (
              <ul className="period-list">{periods.map((period) => <li className={`period-row ${period.is_teaching ? '' : 'period-row--break'}`} key={period.index}>

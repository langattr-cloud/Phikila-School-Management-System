...
export function MyTimetablePage() {
  ...
  useEffect(() => {
    let active = true
    Promise.all([scheduling.me(), scheduling.classes(), scheduling.events(), scheduling.amendments()])
      .then(([principal, classRows, eventRows, amendmentRows]) => {
        if (!active) return
  ...
  if (loading && !view) return <><PageHeader title="My Timetable" description="Your personal timetable and classes where you are the class teacher." /><div className="card section"><LoadingBlock label="Loading your timetable" rows={7} /></div></>
  if (error && !view) return <><PageHeader title="My Timetable" /><ErrorState title="Timetable could not load" message={error} /></>

  return <>
    <PageHeader title="My Timetable" description="View your personal timetable or a class timetable for a class where you are the assigned class teacher." breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'My Timetable' }]} />

    <section className="card section">
      <div className="toolbar">
        <div>
          <p className="eyebrow">VIEW</p>
          <h2 className="section__title">Select timetable</h2>
          <p className="section__description">Choose your personal timetable or a class you are assigned to as class teacher.</p>
        </div>
      ...
      <div className="timetable-controls-row">
        ...
        {kind === 'class' && <div className="timetable-control-group">
          <label className="field__label" htmlFor="my-class-target">Class</label>
    ...
    {view?.version && targetId !== null ? <section className="card section">
      <div className="toolbar"><div><h2 className="section__title">{kind === 'teacher' ? 'My Timetable' : 'Class Timetable'}</h2><p className="section__description">{typeName}{view.target_name ? ` · ${view.target_name}` : ''}</p></div><Badge tone="success">Published</Badge></div>
      <PublishedTimetableGridWithEvents view={view} mode={kind} events={events} />
    </section> : <section className="card section"><EmptyState title={kind === 'class' ? 'No class selected' : 'No timetable available'} description={kind === 'class' ? 'You can only select classes where you are assigned as class teacher.' : 'If the published timetable is missing, generate and publish one from the timetable builder.'} icon={<CalendarIcon width={22} height={22} />} /></section>}
  </>
}

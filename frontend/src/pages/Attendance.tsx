...
import { attendance, type AttendanceSession, type AttendanceContext } from '../lib/attendance'
...
export default function AttendancePage() {
  ...
  async function selectSession(session: AttendanceSession) {
    ...
    try {
      ...
      setError(friendlyApiError(err, 'load stream students'))
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title="Attendance"
        description="Track student attendance by academic year, level, grade, stream and date."
        actions={<button className="button button--primary button--sm" onClick={() => setShowNewSession(!showNewSession)}>{showNewSession ? '✕ Close' : '+ Open Register'}</button>}
      />
      ...
      {loading ? <LoadingBlock label="Loading attendance" rows={4} /> : !sessions.length ? (
        <EmptyState title="No attendance sessions" description="Open a register for an academic stream to start tracking attendance." />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          {sessions.map((s) => (
            <div key={s.id} className="card" style={{ padding: 'var(--space-3)', cursor: 'pointer' }} onClick={() => selectSession(s)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <strong>Year #{s.academic_year_id} · Level #{s.level_id} · Grade #{s.grade_id} · Stream #{s.stream_id}</strong> — {s.date}
                  <span style={{ color: 'var(--color-ink-muted)', fontSize: '0.85rem', marginLeft: 'var(--space-2)' }}>{s.records?.length || 0} records</span>
                </div>
  ...
  return (
    <div className="card section" style={{ marginBottom: 'var(--space-4)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        {[["Academic Year", academicYearId, setAcademicYearId], ["Level", levelId, setLevelId], ["Grade", gradeId, setGradeId], ["Stream", streamId, setStreamId]].map(([label, value, setter]) => (
          <div className="field" key={String(label)}><label className="field__label">{String(label)}</label><input className="input" type="number" value={String(value)} onChange={(e) => (setter as (v: string) => void)(e.target.value)} placeholder="ID" /></div>
        ))}
        ...
        <button className="button button--primary" disabled={!ready} onClick={() => onSubmit({ academic_year_id: Number(academicYearId), level_id: Number(levelId), grade_id: Number(gradeId), stream_id: Number(streamId) }, date)}>Open</button>
  ...
  return (
    <div className="card section" style={{ marginBottom: 'var(--space-4)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-3)' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-2)' }}><button className="button button--primary button--sm" onClick={onBulkPresent}>✓ All Present</button><button className="button button--ghost button--sm" onClick={onClose}>✕</button></div>
      </div>
      {studs.length === 0 ? <p style={{ color: 'var(--color-ink-muted)' }}>No students in this stream.</p> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {studs.map((s) => { const record = session.records?.find((r) => r.student_id === s.id); return (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-2)', border: '1px solid var(--color-line)', borderRadius: 'var(--radius-md)' }}>
              ...
              <div style={{ display: 'flex', gap: 'var(--space-1)' }}>{['present', 'absent', 'late', 'excused'].map((st) => <button key={st} className={`button button--sm ${record?.status === st ? 'button--primary' : 'button--ghost'}`} onClick={() => onMark(s.id, st)}>{st.charAt(0).toUpperCase()}</button>)}</div>
            </div>
          )})}
        </div>
      )}

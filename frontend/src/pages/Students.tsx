        ...
        <>
            ...
            <table style={{ width: '100%', fontSize: '0.875rem', borderCollapse: 'collapse' }}>
              ...
              <tbody>{data.items.map((s) => (
                <tr key={s.id} style={{ borderBottom: '1px solid var(--color-line)' }}>
                  ...
                  <td style={{ padding: 'var(--space-2)' }}><button className="button button--ghost button--sm" onClick={() => setSelectedStudent(s)}>View</button></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
          {data.pages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 'var(--space-2)', marginTop: 'var(--space-4)' }}>
              ...
              <span style={{ padding: 'var(--space-2)', fontSize: '0.875rem', color: 'var(--color-ink-muted)' }}>Page {page} of {data.pages}</span>
...
function StudentForm({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
  const [form, setForm] = useState({
    ...
    date_of_birth: '', gender: '', email: '', phone: '', nationality: 'Kenyan', national_id: '',
    photo_url: '', admission_date: '', academic_year_id: '', level_id: '', grade_id: '', stream_id: '', status: 'active',
  })
  ...
  useEffect(() => {
    ...
    Promise.all([api.academicYears(), api.levels()])
      ...
      .catch((err) => { if (active) setError(friendlyApiError(err, 'load academic options')) })
  ...
  useEffect(() => {
    ...
    api.grades(Number(form.level_id))
      .then((items) => { if (active) setGrades(items) })
  ...
  useEffect(() => {
    ...
    api.streams(Number(form.academic_year_id), Number(form.grade_id))
  ...
  return (
    <div className="card section" style={{ marginBottom: 'var(--space-4)' }}>
      ...
      {error && <Alert tone="error">{error}</Alert>}
      {loadingOptions ? <LoadingBlock label="Loading academic options" rows={2} /> : (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            <Field label="Admission Number *"><input className="input" required value={form.admission_number} onChange={(e) => update('admission_number', e.target.value)} /></Field>
            <Field label="First Name *"><input className="input" required value={form.first_name} onChange={(e) => update('first_name', e.target.value)} /></Field>
            <Field label="Middle Name"><input className="input" value={form.middle_name} onChange={(e) => update('middle_name', e.target.value)} /></Field>
            <Field label="Last Name *"><input className="input" required value={form.last_name} onChange={(e) => update('last_name', e.target.value)} /></Field>
            <Field label="Preferred Name"><input className="input" value={form.preferred_name} onChange={(e) => update('preferred_name', e.target.value)} /></Field>
          </div>
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            <Field label="Date of Birth"><input className="input" type="date" value={form.date_of_birth} onChange={(e) => update('date_of_birth', e.target.value)} /></Field>
            <Field label="Gender"><select className="input" value={form.gender} onChange={(e) => update('gender', e.target.value)}><option value="">Select…</option><option value="Male">Male</option><option value="Female">Female</option></select></Field>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            <Field label="Nationality"><input className="input" value={form.nationality} onChange={(e) => update('nationality', e.target.value)} /></Field>
            <Field label="National ID"><input className="input" value={form.national_id} onChange={(e) => update('national_id', e.target.value)} /></Field>
            <Field label="Photo URL"><input className="input" type="url" value={form.photo_url} onChange={(e) => update('photo_url', e.target.value)} /></Field>
            <Field label="Admission Date"><input className="input" type="date" value={form.admission_date} onChange={(e) => update('admission_date', e.target.value)} /></Field>
          </div>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
            <button className="button button--primary" type="submit" disabled={submitting || loadingGrades || loadingStreams}>{submitting ? 'Saving…' : 'Admit Student'}</button>
  ...
  return (
    <div className="card section" style={{ marginBottom: 'var(--space-4)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-3)' }}>
        <h2 className="section__title" style={{ marginBottom: 0 }}>{student.first_name} {student.last_name}</h2>
      ...
      <dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(14rem, 1fr))', gap: 'var(--space-3)' }}>
        <div><dt>Admission No</dt><dd style={{ fontWeight: 600 }}>{student.admission_number}</dd></div>
        <div><dt>Gender</dt><dd>{student.gender || '—'}</dd></div>
        <div><dt>DOB</dt><dd>{student.date_of_birth || '—'}</dd></div>
        <div><dt>Email</dt><dd>{student.email || '—'}</dd></div>
        <div><dt>Phone</dt><dd>{student.phone || '—'}</dd></div>
        <div><dt>Status</dt><dd><Badge tone={student.status === 'active' ? 'success' : 'warning'}>{student.status}</Badge></dd></div>
      </dl>

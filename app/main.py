...
from app.modules.scheduling.lesson_router import router as timetable_lesson_router
...
def _rate_limit_mutations(router)->None:
    for route in router.routes:
        if not getattr(route,"methods",set()) & {"POST","PUT","PATCH","DELETE"}:continue
        if any(getattr(dep,"dependency",None) is rate_limit_platform_mutation for dep in getattr(route,"dependencies",[])):continue
...
def create_app()->FastAPI:
    ...
    def health_check():
        ...
        return {"status":"ok","environment":settings.environment,"solver_available":ORTOOLS_AVAILABLE}
    ...
    app.include_router(timetable_lesson_router,prefix="/api/v1/scheduling",tags=["Scheduling Lessons"],dependencies=[Depends(rate_limit_scheduling_mutation)])

...
def list_students(
    ...
    if level_id:
        query = query.filter(m.Student.level_id == level_id)
    if stream_id:
        query = query.filter(m.Student.stream_id == stream_id)
    if admission_number:
    ...
    return s.StudentListResponse(
        items=unique, total=total, page=page, page_size=page_size, pages=pages,
    )
...
def create_student(
    ...
    """Admit a new student with optional guardians."""
    ...
    if existing:
        raise HTTPException(
            ...
            f"Admission number '{payload.admission_number}' already exists.",
        )

    student = m.Student(
        school_id=principal.school_id,
        **payload.model_dump(exclude={"guardians"}),
    )
    db.add(student)
    db.flush()
...
def update_student(
    ...
    _audit(db, principal, "update", "student", student.id,
           f"Updated student {student.first_name} {student.last_name}", before, after)
    db.commit()
    db.refresh(student)
    return student
...
def delete_student(
    ...
    student = _get_student_or_404(db, principal.school_id, student_id)
    _audit(db, principal, "delete", "student", student.id,
           f"Deleted student {student.first_name} {student.last_name} ({student.admission_number})")
    db.delete(student)
    db.commit()
...
def list_guardians(
    ...
    return (
        db.query(m.StudentGuardian)
        .filter(m.StudentGuardian.student_id == student_id, m.StudentGuardian.school_id == principal.school_id)
        .all()
    )
...
def add_guardian(
    student_id: int,
    payload: s.GuardianCreate,
    db: Session = Depends(get_db),
    principal: Principal = Depends(require_role("admin", "scheduler")),
):
    _get_student_or_404(db, principal.school_id, student_id)
    guardian = m.StudentGuardian(
        school_id=principal.school_id,
        student_id=student_id,
        **payload.model_dump(),
    )
    db.add(guardian)
    _audit(db, principal, "create", "guardian", student_id,
           f"Added guardian {payload.full_name} for student #{student_id}")
    db.commit()
    db.refresh(guardian)
    return guardian
...
def list_enrollments(
    ...
    _get_student_or_404(db, principal.school_id, student_id)
    return (
        db.query(m.StudentEnrollment)
        .filter(m.StudentEnrollment.student_id == student_id, m.StudentEnrollment.school_id == principal.school_id)
        .order_by(m.StudentEnrollment.created_at.desc())
        .all()
    )
...
def list_documents(
    ...
    _get_student_or_404(db, principal.school_id, student_id)
    return (
        db.query(m.StudentDocument)
        .filter(m.StudentDocument.student_id == student_id, m.StudentDocument.school_id == principal.school_id)
        .order_by(m.StudentDocument.created_at.desc())
        .all()
    )
...
def add_document(
    ...
    db.add(doc)
    _audit(db, principal, "create", "document", student_id,
           f"Added document '{payload.title}' for student #{student_id}")
    db.commit()
    db.refresh(doc)
    return doc

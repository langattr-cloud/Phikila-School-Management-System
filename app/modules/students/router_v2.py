...
def create_student(payload:s.StudentCreate,db:Session=Depends(get_db),principal:Principal=Depends(require_role("admin","scheduler"))):
    ...
    school_class,term=_validate_academic_context(db,principal.school_id,payload.academic_year_id,payload.term_id,payload.level_id,payload.class_id)
    ...
    db.add(student);db.flush();db.add(m.StudentEnrollment(school_id=principal.school_id,student_id=student.id,academic_year_id=payload.academic_year_id,term_id=term.id if term else None,level_id=payload.level_id,school_class_id=school_class.id,class_id=school_class.id,status="active",enrollment_date=payload.admission_date or date.today()))
...
def list_guardians(student_id:int,db:Session=Depends(get_db),principal:Principal=Depends(require_role("viewer","teacher","admin"))):_get_student_or_404(db,principal.school_id,student_id);return db.query(m.StudentGuardian).filter(m.StudentGuardian.student_id==student_id,m.StudentGuardian.school_id==principal.school_id).all()
...
def list_enrollments(student_id:int,db:Session=Depends(get_db),principal:Principal=Depends(require_role("viewer","teacher","admin"))):_get_student_or_404(db,principal.school_id,student_id);return db.query(m.StudentEnrollment).filter(m.StudentEnrollment.student_id==student_id,m.StudentEnrollment.school_id==principal.school_id).order_by(m.StudentEnrollment.created_at.desc()).all()
@router.get("/students/{student_id}/documents",response_model=list[s.DocumentResponse])
def list_documents(student_id:int,db:Session=Depends(get_db),principal:Principal=Depends(require_role("viewer","teacher","admin"))):_get_student_or_404(db,principal.school_id,student_id);return db.query(m.StudentDocument).filter(m.StudentDocument.student_id==student_id,m.StudentDocument.school_id==principal.school_id).order_by(m.StudentDocument.created_at.desc()).all()

...
def list_students(
    ...
    search: str | None = Query(default=None, max_length=200), status_filter: str | None = Query(default=None, alias="status"),
    academic_year_id: int | None = Query(default=None), level_id: int | None = Query(default=None),
    grade_id: int | None = Query(default=None), stream_id: int | None = Query(default=None),
    admission_number: str | None = Query(default=None), db: Session = Depends(get_db),
    ...
    for student in items:
        if student.id not in seen:
            seen.add(student.id); unique.append(student)
...
def create_student(payload: s.StudentCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    for guardian in payload.guardians:
        db.add(m.StudentGuardian(school_id=principal.school_id, student_id=student.id, **guardian.model_dump()))
...
def add_guardian(student_id: int, payload: s.GuardianCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    db.add(guardian); _audit(db, principal, "create", "guardian", student_id, f"Added guardian {payload.full_name} for student #{student_id}")
    db.commit(); db.refresh(guardian); return guardian
...
def add_document(student_id: int, payload: s.DocumentCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler", "teacher"))):
    _get_student_or_404(db, principal.school_id, student_id)
    document = m.StudentDocument(school_id=principal.school_id, student_id=student_id, uploaded_by=principal.user_id, **payload.model_dump())
    db.add(document); _audit(db, principal, "create", "document", student_id, f"Added document '{payload.title}' for student #{student_id}")

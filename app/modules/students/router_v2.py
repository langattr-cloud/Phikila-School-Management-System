...
def _audit(db: Session, principal: Principal, action: str, entity: str, entity_id: int, summary: str, before: dict | None = None, after: dict | None = None) -> None:
...
def _validate_academic_context(db: Session, school_id: int, academic_year_id: int, level_id: int, grade_id: int, stream_id: int):
    from app.modules.academics.models import AcademicYear, Grade, Level, Stream
    if not db.query(AcademicYear.id).filter(AcademicYear.id == academic_year_id, AcademicYear.school_id == school_id).first():
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Academic year does not belong to this school.")
    if not db.query(Level.id).filter(Level.id == level_id, Level.school_id == school_id).first():
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Level does not belong to this school.")
    if not db.query(Grade.id).filter(Grade.id == grade_id, Grade.school_id == school_id, Grade.level_id == level_id).first():
    ...
    if not stream:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Stream does not belong to the selected Academic Year → Level → Grade.")
    return stream

@router.get("/students", response_model=s.StudentListResponse)
def list_students(page: int = Query(default=1, ge=1), page_size: int = Query(default=20, ge=1, le=100), search: str | None = Query(default=None, max_length=200), status_filter: str | None = Query(default=None, alias="status"), academic_year_id: int | None = Query(default=None), level_id: int | None = Query(default=None), grade_id: int | None = Query(default=None), stream_id: int | None = Query(default=None), admission_number: str | None = Query(default=None), db: Session = Depends(get_db), principal: Principal = Depends(require_role("viewer", "teacher", "admin"))):
    query = db.query(m.Student).filter(m.Student.school_id == principal.school_id)
    if search:
        pattern = f"%{search.strip().lower()}%"
        query = query.filter((func.lower(m.Student.first_name).like(pattern)) | (func.lower(m.Student.last_name).like(pattern)) | (func.lower(m.Student.admission_number).like(pattern)) | (func.lower(m.Student.middle_name).like(pattern)))
    ...
    academic_filters = []
    if academic_year_id is not None: academic_filters.append(m.StudentEnrollment.academic_year_id == academic_year_id)
    if level_id is not None: academic_filters.append(m.StudentEnrollment.level_id == level_id)
    if grade_id is not None: academic_filters.append(m.StudentEnrollment.grade_id == grade_id)
    if stream_id is not None: academic_filters.append(m.StudentEnrollment.stream_id == stream_id)
    if academic_filters:
        academic_filters.append(m.StudentEnrollment.status == "active")
        query = query.filter(m.Student.enrollments.any(*academic_filters))
    ...
    for student in items:
        if student.id not in seen: seen.add(student.id); unique.append(student)
...
def create_student(payload: s.StudentCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    student = m.Student(school_id=principal.school_id, level_id=payload.level_id, grade_id=payload.grade_id, stream_id=payload.stream_id, admission_number=payload.admission_number, first_name=payload.first_name, middle_name=payload.middle_name, last_name=payload.last_name, preferred_name=payload.preferred_name, date_of_birth=payload.date_of_birth, gender=payload.gender, email=payload.email, phone=payload.phone, address=payload.address, nationality=payload.nationality, national_id=payload.national_id, photo_url=payload.photo_url, admission_date=payload.admission_date, status=payload.status)
    db.add(student); db.flush()
    enrollment = m.StudentEnrollment(school_id=principal.school_id, student_id=student.id, academic_year_id=payload.academic_year_id, level_id=payload.level_id, grade_id=payload.grade_id, stream_id=stream.id, status="active", enrollment_date=payload.admission_date or date.today())
    db.add(enrollment)
    ...
    _audit(db, principal, "create", "student", student.id, f"Admitted student {payload.first_name} {payload.last_name} ({payload.admission_number})")

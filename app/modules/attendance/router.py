"""Attendance management API — school-scoped and enrollment-context aware."""
...
def _validate_stream_context(db: Session, principal: Principal, academic_year_id: int, level_id: int, grade_id: int, stream_id: int):
    from app.modules.academics.models import AcademicYear, Grade, Level, Stream
    year = db.query(AcademicYear).filter(AcademicYear.id == academic_year_id, AcademicYear.school_id == principal.school_id).first()
    level = db.query(Level).filter(Level.id == level_id, Level.school_id == principal.school_id).first()
    grade = db.query(Grade).filter(Grade.id == grade_id, Grade.school_id == principal.school_id, Grade.level_id == level_id).first()
    stream = db.query(Stream).filter(
        Stream.id == stream_id,
        Stream.school_id == principal.school_id,
        Stream.academic_year_id == academic_year_id,
        Stream.grade_id == grade_id,
        Stream.level_id == level_id,
    ).first()
    if not year or not level or not grade or not stream:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Invalid Academic Year → Level → Grade → Stream context.")
    return stream


@router.post("/attendance/sessions", response_model=s.AttendanceSessionResponse, status_code=201)
def open_attendance_session(
    payload: s.AttendanceSessionCreate,
    db: Session = Depends(get_db),
    principal: Principal = Depends(require_role("admin", "scheduler", "teacher")),
):
    """Open an attendance register for one academic stream on a date."""
    ...
    existing = db.query(m.AttendanceSession).filter(
        m.AttendanceSession.school_id == principal.school_id,
        m.AttendanceSession.academic_year_id == payload.academic_year_id,
        m.AttendanceSession.stream_id == payload.stream_id,
        m.AttendanceSession.date == payload.date,
    ).first()
    if existing:
    ...
    db.add(session)
    _audit(db, principal, "create", "attendance_session", 0,
           f"Opened attendance for {stream.name} on {payload.date}")
    db.commit()
    db.refresh(session)
    return session
...
def list_attendance_sessions(
    ...
    if academic_year_id:
        query = query.filter(m.AttendanceSession.academic_year_id == academic_year_id)
    if level_id:
        query = query.filter(m.AttendanceSession.level_id == level_id)
    if grade_id:
        query = query.filter(m.AttendanceSession.grade_id == grade_id)
    if stream_id:
        query = query.filter(m.AttendanceSession.stream_id == stream_id)
    if date_from:
        query = query.filter(m.AttendanceSession.date >= date_from)
    if date_to:
        query = query.filter(m.AttendanceSession.date <= date_to)
...
def mark_attendance(
    session_id: int,
    payload: s.AttendanceRecordCreate,
    db: Session = Depends(get_db),
    principal: Principal = Depends(require_role("admin", "scheduler", "teacher")),
):
    ...
    if session.status == "closed":
        raise HTTPException(status.HTTP_409_CONFLICT, "This attendance session is closed.")
    ...
    enrollment = db.query(StudentEnrollment).filter(
        ...
        StudentEnrollment.level_id == session.level_id,
        StudentEnrollment.grade_id == session.grade_id,
        StudentEnrollment.stream_id == session.stream_id,
        StudentEnrollment.status == "active",
    ).first()
    if enrollment is None:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Student is not actively enrolled in this academic stream.")

    existing = db.query(m.AttendanceRecord).filter(
        m.AttendanceRecord.session_id == session_id,
        m.AttendanceRecord.student_id == payload.student_id,
    ).first()
    if existing:
...
def bulk_mark(
    ...
    for sid in payload.student_ids:
        ...
        existing = db.query(m.AttendanceRecord).filter(
            m.AttendanceRecord.session_id == session_id,
            m.AttendanceRecord.student_id == sid,
        ).first()
        if existing:
            existing.status = payload.status
            existing.marked_by = principal.user_id
            ...
            db.add(m.AttendanceRecord(
                school_id=principal.school_id, session_id=session_id,
                student_id=sid, status=payload.status, marked_by=principal.user_id,
            ))
        count += 1
    db.commit()
    return {"marked": count, "status": payload.status}


@router.patch("/attendance/records/{record_id}", response_model=s.AttendanceRecordResponse)
def update_record(
    record_id: int,
    payload: s.AttendanceRecordUpdate,
    db: Session = Depends(get_db),
    principal: Principal = Depends(require_role("admin", "scheduler", "teacher")),
):
    record = db.query(m.AttendanceRecord).filter(
        m.AttendanceRecord.id == record_id,
        m.AttendanceRecord.school_id == principal.school_id,
    ).first()
    if not record:
...
def student_attendance_summary(
    ...
    if academic_year_id:
        session_q = session_q.filter(m.AttendanceSession.academic_year_id == academic_year_id)
    session_ids = [row.id for row in session_q.all()]
    if not session_ids:
    ...
    records = db.query(m.AttendanceRecord).filter(
        m.AttendanceRecord.student_id == student_id,
        m.AttendanceRecord.session_id.in_(session_ids),
    ).all()
    total = len(records)
    ...
    return s.AttendanceSummary(
        student_id=student_id,
        student_name=f"{student.first_name} {student.last_name}",
        total_days=total, present=present, absent=absent, late=late, excused=excused,

"""Attendance management API — school-scoped."""
...
def open_attendance_session(
    ...
    """Open an attendance register for a class on a date."""
    existing = (
        db.query(m.AttendanceSession)
        .filter(
            m.AttendanceSession.school_id == principal.school_id,
            m.AttendanceSession.class_id == payload.class_id,
            m.AttendanceSession.date == payload.date,
        )
        .first()
    )
    if existing:
        raise HTTPException(status.HTTP_409_CONFLICT, "Attendance session already exists for this class/date.")
    ...
    _audit(db, principal, "create", "attendance_session", 0,
           f"Opened attendance for class #{payload.class_id} on {payload.date}")
    db.commit()
    db.refresh(session)
    return session
...
def list_attendance_sessions(
    ...
    if class_id:
        query = query.filter(m.AttendanceSession.class_id == class_id)
    if date_from:
...
def mark_attendance(
    ...
    """Mark a single student's attendance for a session."""
    ...
    existing = (
        db.query(m.AttendanceRecord)
        .filter(m.AttendanceRecord.session_id == session_id, m.AttendanceRecord.student_id == payload.student_id)
        .first()
    )
    if existing:
...
def bulk_mark(
    ...
    """Mark all listed students with the same status."""
    session = db.query(m.AttendanceSession).filter(
        m.AttendanceSession.id == session_id,
        m.AttendanceSession.school_id == principal.school_id,
    ).first()
    ...
    for sid in payload.student_ids:
        existing = (
            db.query(m.AttendanceRecord)
            .filter(m.AttendanceRecord.session_id == session_id, m.AttendanceRecord.student_id == sid)
            .first()
        )
        if existing:
            existing.status = payload.status
...
def student_attendance_summary(
    ...
    session_ids = [s.id for s in session_q.all()]
    if not session_ids:
        return s.AttendanceSummary(
            student_id=student_id, student_name=f"{student.first_name} {student.last_name}",
            total_days=0, present=0, absent=0, late=0, excused=0, attendance_rate=0,
        )

    records = (
        db.query(m.AttendanceRecord)
        .filter(m.AttendanceRecord.student_id == student_id, m.AttendanceRecord.session_id.in_(session_ids))
        .all()
    )

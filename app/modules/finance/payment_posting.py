...
def post_fee_payment(db: Session, *, school_id: int, invoice: m.StudentInvoice, student_id: int,
                     ...
                     notes: str | None, actor: str | None, journal_number: str,
                     receipt_number: str) -> tuple[m.Payment, m.FinanceReceipt]:
    ...
    journal = post_journal(db, school_id=school_id, journal_number=journal_number,
                           description=f"Fee payment for invoice #{invoice.id}",
                           reference=reference_number, created_by=actor,
    ...
    receipt = m.FinanceReceipt(school_id=school_id, receipt_number=receipt_number, payment_id=payment.id,

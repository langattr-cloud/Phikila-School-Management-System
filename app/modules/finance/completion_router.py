...
def create_cash_book(payload: dict, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin'))):
    if not payload.get('name') or not payload.get('book_type'):
        raise HTTPException(400, 'name and book_type are required')
    book = o.FinanceCashBook(school_id=principal.school_id, name=payload['name'],
                             book_type=payload['book_type'], bank_account_id=payload.get('bank_account_id'),
...
def add_cash_book_entry(cash_book_id: int, payload: dict, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin','scheduler'))):
    ...
    entry = o.FinanceCashBookEntry(school_id=principal.school_id, cash_book_id=book.id,
        entry_date=payload.get('entry_date', datetime.now(timezone.utc)), entry_type=payload['entry_type'],
        amount=money(payload['amount']), reference=payload.get('reference'), description=payload.get('description'),
        source_entity=payload.get('source_entity'), source_id=payload.get('source_id'), created_by=principal.user_id)
...
def reconcile_bank(payload: dict, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin'))):
    ...
    book_balance = money(payload.get('book_balance'))
    statement_balance = money(payload['statement_balance'])
    row = o.FinanceBankReconciliation(school_id=principal.school_id, bank_account_id=bank.id,
        statement_date=payload['statement_date'], statement_balance=statement_balance, book_balance=book_balance,
        difference=statement_balance-book_balance, status='RECONCILED' if statement_balance == book_balance else 'OPEN',
        reconciled_by=principal.user_id if statement_balance == book_balance else None,
        reconciled_at=datetime.now(timezone.utc) if statement_balance == book_balance else None, notes=payload.get('notes'))
...
async def import_bank_statement(bank_account_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin','scheduler'))):
    ...
    for row in reader:
        ...
        if ref and db.query(o.FinanceBankTransaction).filter_by(school_id=principal.school_id, bank_account_id=bank.id, external_reference=ref).first():
        ...
        try: tx_date = datetime.fromisoformat(normalized['date'].replace('Z','+00:00'))
        except ValueError:
            try: tx_date = datetime.strptime(normalized['date'], '%Y-%m-%d')
            except ValueError: raise HTTPException(400, f'Invalid date for reference {ref or "unknown"}')
        tx = o.FinanceBankTransaction(school_id=principal.school_id, bank_account_id=bank.id,
            transaction_date=tx_date, value_date=tx_date, amount=amount,
            transaction_type=str(normalized['type']).upper(), external_reference=ref,
    ...
    db.commit()
    return {'bank_account_id': bank.id, 'filename': file.filename, 'imported': imported, 'duplicates': duplicates}

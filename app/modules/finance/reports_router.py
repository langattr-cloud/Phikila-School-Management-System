...
def trial_balance(db: Session = Depends(get_db), principal: Principal = Depends(require_role('viewer', 'admin'))):
    rows = (
        db.query(
            m.ChartOfAccount.id,
            m.ChartOfAccount.code,
            m.ChartOfAccount.name,
            m.ChartOfAccount.account_type,
            func.coalesce(func.sum(m.JournalEntry.debit), 0),
            func.coalesce(func.sum(m.JournalEntry.credit), 0),
        )
...
def balance_sheet(db: Session = Depends(get_db), principal: Principal = Depends(require_role('viewer', 'admin'))):
    rows = trial_balance(db, principal)

    def normal_balance(row):
        account_type = str(row['account_type']).upper()
        if account_type in {'ASSET', 'EXPENSE', 'EXPENDITURE'}:
            return row['debit'] - row['credit']
        return row['credit'] - row['debit']

    assets = [r for r in rows if str(r['account_type']).upper() == 'ASSET']
    liabilities = [r for r in rows if str(r['account_type']).upper() == 'LIABILITY']
    equity = [r for r in rows if str(r['account_type']).upper() in {'EQUITY', 'NET_ASSET', 'NET_ASSETS', 'FUND_BALANCE'}]
    ...
    current_surplus_deficit = income - expenditure

    asset_total = sum((normal_balance(r) for r in assets), Decimal('0'))
    liability_total = sum((normal_balance(r) for r in liabilities), Decimal('0'))
    equity_total = sum((normal_balance(r) for r in equity), Decimal('0'))
    net_assets = equity_total + current_surplus_deficit

    return {
        'assets': [{**r, 'balance': normal_balance(r)} for r in assets],
        'liabilities': [{**r, 'balance': normal_balance(r)} for r in liabilities],
        'equity': [{**r, 'balance': normal_balance(r)} for r in equity],
        ...
        'totals': {
            ...
            'liabilities_and_net_assets': liability_total + net_assets,
            'balance_check': asset_total - (liability_total + net_assets),
        },
    }
...
def debtors(db: Session = Depends(get_db), principal: Principal = Depends(require_role('viewer', 'teacher', 'admin'))):
    rows = fee_collection(db, principal)
    return sorted([r for r in rows if r['outstanding'] > 0], key=lambda x: x['outstanding'], reverse=True)

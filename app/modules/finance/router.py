...
def list_payment_inbox(status_filter: str | None = Query(default=None, alias="status"), db: Session = Depends(get_db), principal: Principal = Depends(require_role("viewer", "admin"))):
    ...
    if status_filter:
        q = q.filter(m.PaymentInbox.status == status_filter)
...
def ingest_payment(payload: s.PaymentInboxCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    received_at = payload.received_at or decoded["received_at"] or datetime.utcnow()
    if not amount or not external_reference:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Payment amount and external reference could not be determined.")

    duplicate = db.query(m.PaymentInbox).filter(
        m.PaymentInbox.school_id == principal.school_id,
        m.PaymentInbox.source == payload.source,
        m.PaymentInbox.external_reference == external_reference,
    ).first()
    if duplicate:
        raise HTTPException(status.HTTP_409_CONFLICT, f"Duplicate payment reference; already received as inbox item #{duplicate.id}.")
    ...
    if student_identifier:
        matched_student = db.query(Student).filter(
            ...
            Student.admission_number == student_identifier,
        ).first()
        if matched_student:
            match_method = "admission_number"
    ...
    item = m.PaymentInbox(
        ...
        account_name=payload.account_name or decoded["account_name"],
        raw_message=payload.raw_message,
        amount=amount,
        external_reference=external_reference,
        student_identifier=student_identifier,
        received_at=received_at,
        payment_channel=payload.payment_channel or decoded["payment_channel"],
    ...
    db.add(item)
    _audit(db, principal, "create", "payment_inbox", 0, f"Ingested external payment {external_reference} — {amount}; status={inbox_status}")
    db.commit()
    db.refresh(item)
    return item
...
def student_balance(student_id: int, db: Session = Depends(get_db), principal: Principal = Depends(require_role("viewer", "teacher", "admin"))):
    ...
    return s.StudentBalance(student_id=student_id, student_name=f"{student.first_name} {student.last_name}", total_invoiced=Decimal(str(total_invoiced)), total_paid=Decimal(str(total_paid)), balance=max(balance, Decimal("0")))
...
def finance_overview(db: Session = Depends(get_db), principal: Principal = Depends(require_role("viewer", "teacher", "admin"))):
    ...
    return s.FinanceOverview(total_invoiced=Decimal(str(total_invoiced)), total_collected=Decimal(str(total_paid)), total_outstanding=max(total_outstanding, Decimal("0")), invoices_count=invoices_count, paid_count=paid_count, pending_count=pending_count)

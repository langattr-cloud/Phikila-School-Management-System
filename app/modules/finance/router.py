...
def record_payment(payload: s.PaymentCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    if payload.reference_number:
        duplicate = db.query(m.Payment).filter(m.Payment.school_id == principal.school_id, m.Payment.reference_number == payload.reference_number).first()
        if duplicate: raise HTTPException(status.HTTP_409_CONFLICT, "Payment reference has already been processed.")
    ...
    invoice.balance = max(Decimal(str(invoice.balance)), Decimal("0"))
...
def ingest_payment(payload: s.PaymentInboxCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    amount = payload.amount or decoded["amount"]
    external_reference = payload.external_reference or decoded["external_reference"]
    student_identifier = payload.student_identifier or decoded["student_identifier"]
    received_at = payload.received_at or decoded["received_at"] or datetime.now(timezone.utc)
    ...
    if student_identifier:
        ...
        if matched_student:
            match_method, confidence, inbox_status = "admission_number", Decimal("100.00"), "MATCHED"

    item = m.PaymentInbox(school_id=principal.school_id, source=payload.source, source_account=payload.source_account,
        account_name=payload.account_name or decoded["account_name"], raw_message=payload.raw_message, amount=amount,
        external_reference=external_reference, student_identifier=student_identifier, received_at=received_at,
        payment_channel=payload.payment_channel or decoded["payment_channel"], matched_student_id=matched_student.id if matched_student else None,
        match_method=match_method, match_confidence=confidence, status=inbox_status)
...
def post_payment_inbox(inbox_id: int, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin"))):
    ...
    invoice = db.query(m.StudentInvoice).filter(m.StudentInvoice.school_id == principal.school_id, m.StudentInvoice.student_id == item.matched_student_id).order_by(m.StudentInvoice.created_at.asc()).first()
    if not invoice: raise HTTPException(status.HTTP_409_CONFLICT, "Matched student has no fee invoice to receive this payment.")
    ...
    duplicate = db.query(m.Payment).filter(m.Payment.school_id == principal.school_id, m.Payment.reference_number == item.external_reference).first()
    if duplicate:
        item.status, item.posted_payment_id, item.posted_at = "DUPLICATE", duplicate.id, datetime.now(timezone.utc)
        _audit(db, principal, "duplicate", "payment_inbox", item.id, f"Payment reference {item.external_reference} already posted as payment #{duplicate.id}")
    ...
    payment = m.Payment(school_id=principal.school_id, invoice_id=invoice.id, student_id=item.matched_student_id,
        ...
        notes=f"Posted from payment inbox #{item.id}", received_by=principal.user_id)
    db.add(payment); db.flush()
    invoice.balance = max(Decimal(str(invoice.balance)) - Decimal(str(item.amount)), Decimal("0"))
    invoice.status = "paid" if invoice.balance == 0 else "partial"
    item.status, item.posted_payment_id, item.posted_at = "POSTED", payment.id, datetime.now(timezone.utc)
    item.reviewed_by, item.reviewed_at = principal.user_id, datetime.now(timezone.utc)

...
The HTTP API creates a durable job row and dispatches the solver outside the
request thread. The solver opens its own SQLAlchemy session, so the web request
never blocks on OR-Tools and polling the job remains responsive.
"""
...
def enqueue(job_id: int, school_id: int, max_seconds: float = 30.0):
    """Dispatch without blocking the FastAPI request thread."""
    return _executor.submit(_run_job, job_id, school_id, max_seconds)
...
def _fail(db,job,message):
    job.status="failed";job.stage="Failed";job.message=message;job.finished_at=datetime.utcnow();db.commit()

...
def create_job(db:Session,school_id:int,actor:str|None,profile_label:str|None=None,profile_day_indexes:list[int]|None=None):
    job=m.TtSolverJob(school_id=school_id,status="queued",stage="Queued",progress=0,checks=initial_checks(),created_by=actor,profile_label=profile_label,profile_day_indexes=profile_day_indexes or [])
...
def _run_job(job_id,school_id,max_seconds):
    ...
    try:
        ...
        profile_days=list(job.profile_day_indexes or [])
        if profile_days:
            # Build the solver against this timetable's selected days without
            # changing the school's shared calendar rows.
            data=build_input(db,school_id,max_seconds=max_seconds,day_indexes=profile_days)
        else:data=build_input(db,school_id,max_seconds=max_seconds)
        ...
        version=_persist(db,school_id,result,job.created_by,job.profile_label,profile_days)
...
def _persist(db,school_id,result,actor,profile_label=None,profile_days=None):
    last=db.query(m.TtVersion).filter(m.TtVersion.school_id==school_id).order_by(m.TtVersion.number.desc()).first()
    days=list(profile_days or [])
    rows={d.index:d for d in db.query(m.TtDay).filter(m.TtDay.school_id==school_id).all()}
    version=m.TtVersion(school_id=school_id,number=(last.number+1) if last else 1,label=profile_label or "Generated",status="draft",quality=result.quality,stats=result.stats,created_by=actor,day_indexes=days or [d.index for d in rows.values() if d.is_active],day_names=[rows[i].name for i in days] if days else [d.name for d in rows.values() if d.is_active])

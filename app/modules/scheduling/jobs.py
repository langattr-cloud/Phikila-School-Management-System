    ...
    try:
        ...
        def report(pct,stage):
            try:
                ...
                if pct>=26:checks=_set_checks(checks,["teacher_conflicts","class_conflicts","room_conflicts","availability"],"passed")
        ...
        if hard_conflicts:
            ...
            return
        db.query(m.TtVersion).filter(m.TtVersion.school_id==school_id,m.TtVersion.status=="published").update({"status":"archived"});version.status="published";version.published_at=utcnow();checks=_set_checks(job.checks or initial_checks(),["teacher_conflicts","class_conflicts","room_conflicts","availability","double_lessons","workload","distribution"],"passed");job.checks=checks;job.status="completed";job.stage="Completed";job.progress=100;job.result_version_id=version.id;job.quality=result.quality;job.finished_at=utcnow();db.commit();db.add(m.TtAuditEntry(school_id=school_id,actor=job.created_by,action="generate",entity="version",entity_id=version.id,summary=f"Generated and published timetable v{version.number} ({version.label or 'Generated'})"));db.commit();logger.info("Solver job %s completed and published version %s",job_id,version.id)
    except Exception:

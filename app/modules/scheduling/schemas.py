...
class GenerateIn(BaseModel):
    ...
    day_indexes:list[int]=Field(default_factory=list,max_length=7)
    @field_validator('day_indexes')
    @classmethod
    def validate_days(cls,v):
        if len(set(v))!=len(v) or any(i<0 or i>6 for i in v): raise ValueError('Select unique working days from Monday to Sunday')
        return v

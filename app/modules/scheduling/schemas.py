...
class TeacherIn(BaseModel):
    name: str = Field(min_length=1, max_length=120); code: str = Field(min_length=1, max_length=30); email: str | None = None; department: str | None = None
    max_lessons_per_day: int = Field(default=7, ge=1, le=20); max_consecutive: int = Field(default=4, ge=1, le=20); workload_target: int | None = Field(default=None, ge=0, le=80)
    unavailable: Slots = Field(default_factory=dict); is_active: bool = True
class TeacherOut(ORMModel, TeacherIn): id: int
class SubjectIn(BaseModel):
    name: str = Field(min_length=1, max_length=120); code: str = Field(min_length=1, max_length=30); colour: str = Field(default='#0F2A47', max_length=9)
    prefers_morning: bool = False; prefers_double: bool = False; spread_across_week: bool = True; required_room_type: str | None = None
...
class GenerateProfileIn(BaseModel):
    max_seconds: float = Field(default=30.0, ge=1.0, le=180.0); label: str = Field(default='New timetable', min_length=1, max_length=120); day_indexes: list[int] = Field(min_length=1, max_length=7)
    @field_validator('day_indexes')
    @classmethod
    def validate_days(cls, value):
        if len(set(value)) != len(value) or any(day < 0 or day > 6 for day in value): raise ValueError('Select unique working days from Monday to Sunday')
        return sorted(value)
...
class LessonOut(ORMModel):
    id: int; version_id: int; requirement_id: int | None; class_id: int; subject_id: int; teacher_id: int | None; room_id: int | None; day_index: int; period_index: int; duration: int; is_locked: bool
class LessonMoveIn(BaseModel): day_index: int = Field(ge=0, le=6); period_index: int = Field(ge=0, le=30); room_id: int | None = None
class LessonPatch(BaseModel):
    day_index: int | None = Field(default=None, ge=0, le=6); period_index: int | None = Field(default=None, ge=0, le=30); duration: int | None = Field(default=None, ge=1, le=10); teacher_id: int | None = None; class_id: int | None = None; subject_id: int | None = None; room_id: int | None = None; is_locked: bool | None = None
class LessonCreate(BaseModel):
    requirement_id: int; day_index: int = Field(ge=0, le=6); period_index: int = Field(ge=0, le=30); duration: int = Field(default=1, ge=1, le=10); room_id: int | None = None
class UnassignedOut(BaseModel):
    requirement_id: int; subject_id: int; subject_name: str; subject_colour: str; class_id: int; class_name: str; teacher_id: int | None; teacher_name: str | None; room_id: int | None; room_name: str | None; periods_per_week: int; placed: int; remaining: int; requires_double: bool
class VersionOut(ORMModel):
    id: int; number: int; label: str | None; status: str; quality: dict[str, Any] = Field(default_factory=dict); stats: dict[str, Any] = Field(default_factory=dict); created_by: str | None; created_at: Any = None; published_at: Any = None; day_indexes: list[int] = Field(default_factory=list); day_names: list[str] = Field(default_factory=list)
...
class EventIn(BaseModel):
    name: str = Field(min_length=1, max_length=80); start_time: str = Field(pattern=r'^\d{2}:\d{2}$'); end_time: str = Field(pattern=r'^\d{2}:\d{2}$'); day_indexes: list[int] = Field(min_length=1, max_length=7); event_type: str = Field(default='break', min_length=1, max_length=40); note: str | None = None
    @field_validator('day_indexes')
    @classmethod
...
class CopilotIn(BaseModel):
    ...
    @classmethod
    def strip(cls, value): return value.strip()
class CopilotApplyIn(BaseModel): command: dict[str, Any]
class ImportRow(BaseModel):
    teacher: str | None = None; subject: str | None = None; klass: str | None = Field(default=None, alias='class'); room: str | None = None; periods_per_week: int | None = None; model_config = ConfigDict(populate_by_name=True)

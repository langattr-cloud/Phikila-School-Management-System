...
class TeacherIn(BaseModel):
    ...
    max_lessons_per_day: int = Field(default=7, ge=1, le=20); max_consecutive: int = Field(default=4, ge=1, le=20); workload_target: int | None = Field(default=None, ge=0, le=80); unavailable: Slots = Field(default_factory=dict); is_active: bool = True
...
class ClassIn(BaseModel):
    name: str = Field(min_length=1, max_length=120); code: str = Field(min_length=1, max_length=30); grade: str | None = None; student_count: int = Field(default=40, ge=1, le=500); home_room_id: int | None = None; unavailable: Slots = Field(default_factory=dict)
...
class GenerateIn(BaseModel):
    max_seconds: float = Field(default=30.0, ge=1.0, le=180.0)
...
class GenerateProfileIn(BaseModel):
    ...
    def validate_days(cls, value: list[int]) -> list[int]:
        if len(set(value)) != len(value) or any(day < 0 or day > 6 for day in value):
            raise ValueError("Select unique working days from Monday to Sunday")
        return sorted(value)
...
class LessonMoveIn(BaseModel):
    day_index: int = Field(ge=0, le=6); period_index: int = Field(ge=0, le=30); room_id: int | None = None
...
class VersionOut(ORMModel):
    id: int; number: int; label: str | None; status: str; quality: dict[str, Any] = Field(default_factory=dict); stats: dict[str, Any] = Field(default_factory=dict); created_by: str | None; created_at: Any = None; published_at: Any = None
...
class CopilotIn(BaseModel):
    ...
    @classmethod
    def strip(cls, value: str) -> str: return value.strip()
class CopilotApplyIn(BaseModel): command: dict[str, Any]
class ImportRow(BaseModel):
    teacher: str | None = None; subject: str | None = None; klass: str | None = Field(default=None, alias="class"); room: str | None = None; periods_per_week: int | None = None

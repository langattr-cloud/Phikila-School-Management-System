...
class PeriodIn(BaseModel):
    index: int = Field(ge=0, le=30); name: str = Field(min_length=1, max_length=40); start_time: str = Field(pattern=r'^\d{2}:\d{2}$'); end_time: str = Field(pattern=r'^\d{2}:\d{2}$'); is_teaching: bool = True
...
class SubjectIn(BaseModel):
    name: str = Field(min_length=1, max_length=120); code: str = Field(min_length=1, max_length=30); colour: str = Field(default='#0F2A47', max_length=9); prefers_double: bool = False; spread_across_week: bool = True; required_room_type: str | None = None

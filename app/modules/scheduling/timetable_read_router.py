...
router = APIRouter()
def _version(db, school_id, version_id): return db.query(m.TtVersion).filter(m.TtVersion.id == version_id, m.TtVersion.school_id == school_id).first()
def _current(db, school_id):
    current_type=db.query(m.TtTimetableType).filter(m.TtTimetableType.school_id==school_id,m.TtTimetableType.is_active.is_(True)).order_by(m.TtTimetableType.id.desc()).first()
    if current_type is None:return None,None
    version=db.query(m.TtVersion).filter(m.TtVersion.school_id==school_id,m.TtVersion.status=='published',m.TtVersion.timetable_type_id==current_type.id).order_by(m.TtVersion.published_at.desc(),m.TtVersion.id.desc()).first()
...
def current_timetable_view(scope:str,target_id:int,db:Session=Depends(get_db),principal:Principal=Depends(resolve_principal)):
    if scope not in {'teacher','class','room'}:raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY,'Invalid timetable scope.')
    timetable_type,version=_current(db,principal.school_id)
    if not version:return {'days':[],'periods':[],'lessons':[],'target_name':None,'version':None,'timetable_type':None}
    days=db.query(m.TtDay).filter(m.TtDay.school_id==principal.school_id,m.TtDay.is_active.is_(True),m.TtDay.index.in_(list(timetable_type.day_indexes or []))).order_by(m.TtDay.index).all()
    period_indexes={int(i) for i in (timetable_type.period_indexes or [])}
    periods=db.query(m.TtPeriod).filter(m.TtPeriod.school_id==principal.school_id).order_by(m.TtPeriod.index).all()
    if period_indexes:periods=[p for p in periods if p.index in period_indexes]
    lessons=db.query(m.TtLesson).filter(m.TtLesson.school_id==principal.school_id,m.TtLesson.version_id==version.id,m.TtLesson.day_index.in_([d.index for d in days]),m.TtLesson.period_index.in_(list(period_indexes))).all()
    ...
    return {'days':[{'index':d.index,'name':d.name} for d in days],'periods':[{'index':p.index,'name':p.name,'start_time':p.start_time.strftime('%H:%M'),'end_time':p.end_time.strftime('%H:%M'),'is_teaching':p.is_teaching} for p in periods],'lessons':[{'day':x.day_index,'period':x.period_index,'subject':subject_map.get(x.subject_id).code if subject_map.get(x.subject_id) else 'Unknown subject','subject_colour':subject_map.get(x.subject_id).colour if subject_map.get(x.subject_id) else None,'teacher':teacher_map.get(x.teacher_id).code if x.teacher_id and teacher_map.get(x.teacher_id) else None,'class':class_map.get(x.class_id).code if class_map.get(x.class_id) else 'Unknown class'} for x in lessons],'target_name':target.name if target else None,'version':s.VersionOut.model_validate(version).model_dump(),'timetable_type':{'id':timetable_type.id,'name':timetable_type.name,'code':timetable_type.code,'display_mode':timetable_type.display_mode,'day_indexes':list(timetable_type.day_indexes or []),'period_indexes':list(timetable_type.period_indexes or []),'is_active':timetable_type.is_active,'is_system':timetable_type.is_system}}
...
def version_unassigned(version_id:int,db:Session=Depends(get_db),principal:Principal=Depends(resolve_principal)):
    version=_version(db,principal.school_id,version_id)
    if not version:raise HTTPException(status.HTTP_404_NOT_FOUND,'Not found')
    requirements=db.query(m.TtLessonRequirement).filter(m.TtLessonRequirement.school_id==principal.school_id).all();scheduled=db.query(m.TtLesson.requirement_id).filter(m.TtLesson.school_id==principal.school_id,m.TtLesson.version_id==version_id).all();counts={}

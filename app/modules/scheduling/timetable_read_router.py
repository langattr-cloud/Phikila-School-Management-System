...
def _version(db, school_id, version_id):
    return db.query(m.TtVersion).filter(
        m.TtVersion.id == version_id,
...
def _current(db, school_id):
    ...
    if current_type is not None:
        typed = query.filter(m.TtVersion.timetable_type_id == current_type.id).order_by(
            m.TtVersion.published_at.desc(), m.TtVersion.id.desc()
        ).first()
        if typed is not None:
            return current_type, typed
    ...
    if current_type is None:
        current_type = (
            db.query(m.TtTimetableType)
            .filter(m.TtTimetableType.school_id == school_id)
            .order_by(m.TtTimetableType.id.desc())
            .first()
        )
...
def _assert_scope_access(db: Session, principal: Principal, scope: str, target_id: int) -> None:
    ...
    if scope == "teacher":
        if target_id != teacher_id:
            raise HTTPException(status.HTTP_403_FORBIDDEN, "A teacher can only view their own timetable.")
        return
...
def current_timetable_view(
    ...
    lesson_query = db.query(m.TtLesson).filter(
    ...
    if scope == 'teacher':
        lessons = [x for x in lessons if x.teacher_id == target_id]
    elif scope == 'class':
        lessons = [x for x in lessons if x.class_id == target_id]
    elif scope == 'room':
        lessons = [x for x in lessons if x.room_id == target_id]
    else:
        lessons = [x for x in lessons if x.subject_id == target_id]
    ...
    room_map = {x.id: x for x in rooms}

    if scope == 'teacher':
        target = teacher_map.get(target_id)
    elif scope == 'class':
        target = class_map.get(target_id)
    elif scope == 'room':
        target = room_map.get(target_id)
    else:
        target = subject_map.get(target_id)
    ...
    return {
        ...
        'lessons': [
            {
                'day': x.day_index,
                'period': x.period_index,
                'subject': subject_map.get(x.subject_id).code if subject_map.get(x.subject_id) else 'Unknown subject',
                'subject_colour': subject_map.get(x.subject_id).colour if subject_map.get(x.subject_id) else None,
                'teacher': teacher_map.get(x.teacher_id).code if x.teacher_id and teacher_map.get(x.teacher_id) else None,
                'class': class_map.get(x.class_id).code if class_map.get(x.class_id) else 'Unknown class',
                'room': room_map.get(x.room_id).code if x.room_id and room_map.get(x.room_id) else None,
            }
            for x in lessons
        ],
        ...
        'timetable_type': (
            {
                'id': timetable_type.id,
                'name': timetable_type.name,
                'code': timetable_type.code,
                'display_mode': timetable_type.display_mode,
                'day_indexes': list(timetable_type.day_indexes or []),
                'period_indexes': list(timetable_type.period_indexes or []),
                'is_active': timetable_type.is_active,
                'is_system': timetable_type.is_system,
            }
            if timetable_type else None
        ),
    }
...
def version_lessons(version_id: int, db: Session = Depends(get_db), principal: Principal = Depends(resolve_principal)):
    ...
    return db.query(m.TtLesson).filter(
        m.TtLesson.school_id == principal.school_id,
        m.TtLesson.version_id == version_id,
    ).order_by(m.TtLesson.day_index, m.TtLesson.period_index, m.TtLesson.id).all()
...
def version_unassigned(version_id: int, db: Session = Depends(get_db), principal: Principal = Depends(resolve_principal)):
    ...
    scheduled = db.query(m.TtLesson.requirement_id).filter(
        m.TtLesson.school_id == principal.school_id,
        m.TtLesson.version_id == version_id,
    ).all()
    counts = {}

...
def _current(db, school_id):
    ...
    version = db.query(m.TtVersion).filter(m.TtVersion.school_id == school_id, m.TtVersion.status == 'published', m.TtVersion.timetable_type_id == current_type.id).order_by(m.TtVersion.number.desc(), m.TtVersion.id.desc()).first()
...
def current_timetable_view(scope: str, target_id: int, db: Session = Depends(get_db), principal: Principal = Depends(resolve_principal)):
    ...
    if not version:
        return {'days': [], 'periods': [], 'lessons': [], 'target_name': None, 'version': None, 'timetable_type': None}
    ...
    lessons = db.query(m.TtLesson).filter(m.TtLesson.school_id == principal.school_id, m.TtLesson.version_id == version.id).all()

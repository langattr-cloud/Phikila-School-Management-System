...
def create_timetable_type(payload: s.TimetableTypeIn, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin','scheduler'))):
    ...
    if db.query(m.TtTimetableType).filter(
        m.TtTimetableType.school_id == principal.school_id,
        m.TtTimetableType.code == code,
    ).first():
        raise HTTPException(status.HTTP_409_CONFLICT, 'A timetable type with that code already exists.')

    configured = _configured_days(db, principal.school_id)
    day_indexes = sorted(set(int(i) for i in payload.day_indexes))
    if not set(day_indexes).issubset(configured):

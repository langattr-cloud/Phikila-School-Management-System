...
def _values(payload: dict, db: Session, school_id: int):
    ...
    if not name or not code:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, 'Timetable type name and code are required.')
    ...
    if not periods or not set(periods).issubset(configured_periods):
        raise HTTPException(status.HTTP_400_BAD_REQUEST, 'One or more selected teaching periods are not configured for this school.')

    display_mode = payload.get('display_mode') or 'day'
    if display_mode not in {'day', 'date'}:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, 'Invalid timetable display mode.')
...
def create_timetable_type(payload: dict, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin', 'scheduler'))):
    """Create or update a timetable type without disabling other active types."""

"""Scheduling generation and school-scoped timetable print profiles."""
...
RECENT_RESULT_WINDOW = timedelta(minutes=30)
...
def _active_job(db, school_id):
    ...
    if job:
        heartbeat=_utc(job.updated_at or job.started_at); stale=job.finished_at is not None or bool(heartbeat and now-heartbeat>timedelta(minutes=10))
        if stale:
...
def _config(db,school_id,payload):
    ...
    if payload.project_id is not None:
        project=db.query(m.TtProject).filter(m.TtProject.id==int(payload.project_id),m.TtProject.school_id==school_id).first()
        if project is None: raise HTTPException(status.HTTP_404_NOT_FOUND,"Timetable project not found.")
        if project.status=="archived": raise HTTPException(status.HTTP_409_CONFLICT,"Archived timetable projects cannot be generated.")
    else:
        projects=db.query(m.TtProject).filter(m.TtProject.school_id==school_id,m.TtProject.status!="archived").order_by(m.TtProject.updated_at.desc(),m.TtProject.id.desc()).limit(2).all()
        if len(projects)==1: project=projects[0]
        elif len(projects)>1: raise HTTPException(status.HTTP_409_CONFLICT,"Select a timetable project before generating when multiple projects exist.")
    ...
    return {"project_id":project.id if project else None,"label":payload.label,"timetable_type_id":current.id,"display_mode":current.display_mode,"day_indexes":day_indexes,"day_names":day_names,"class_ids":payload.class_ids,"teacher_ids":payload.teacher_ids,"period_indexes":period_indexes}

@router.get("/print-profiles",response_model=list[s.PrintProfileOut])
def list_print_profiles(db:Session=Depends(get_db),principal:Principal=Depends(resolve_principal)):
    return db.query(m.TtPrintProfile).filter(m.TtPrintProfile.school_id==principal.school_id).order_by(m.TtPrintProfile.is_default.desc(),m.TtPrintProfile.name.asc()).all()
@router.post("/print-profiles",response_model=s.PrintProfileOut,status_code=201)
def create_print_profile(payload:s.PrintProfileIn,db:Session=Depends(get_db),principal:Principal=Depends(require_role("admin","scheduler"))):
    if payload.is_default: db.query(m.TtPrintProfile).filter(m.TtPrintProfile.school_id==principal.school_id).update({"is_default":False},synchronize_session=False)
    row=m.TtPrintProfile(school_id=principal.school_id,name=payload.name.strip(),config=payload.config,is_default=payload.is_default,created_by=principal.email or principal.user_id); db.add(row)
    try: db.commit()
    except Exception: db.rollback(); raise HTTPException(status.HTTP_409_CONFLICT,"A print profile with that name already exists.")
    db.refresh(row); return row
@router.put("/print-profiles/{profile_id}",response_model=s.PrintProfileOut)
def update_print_profile(profile_id:int,payload:s.PrintProfileIn,db:Session=Depends(get_db),principal:Principal=Depends(require_role("admin","scheduler"))):
    row=db.query(m.TtPrintProfile).filter(m.TtPrintProfile.id==profile_id,m.TtPrintProfile.school_id==principal.school_id).first()
    if row is None: raise HTTPException(status.HTTP_404_NOT_FOUND,"Print profile not found.")
    if payload.is_default: db.query(m.TtPrintProfile).filter(m.TtPrintProfile.school_id==principal.school_id,m.TtPrintProfile.id!=row.id).update({"is_default":False},synchronize_session=False)
    row.name=payload.name.strip(); row.config=payload.config; row.is_default=payload.is_default; db.commit(); db.refresh(row); return row
@router.delete("/print-profiles/{profile_id}",status_code=204)
def delete_print_profile(profile_id:int,db:Session=Depends(get_db),principal:Principal=Depends(require_role("admin","scheduler"))):
    row=db.query(m.TtPrintProfile).filter(m.TtPrintProfile.id==profile_id,m.TtPrintProfile.school_id==principal.school_id).first()
    if row is None: raise HTTPException(status.HTTP_404_NOT_FOUND,"Print profile not found.")
...
def generate_async(payload:s.GenerateIn,db:Session=Depends(get_db),principal:Principal=Depends(require_role("admin","scheduler"))):
    ...
    current=_active_job(db,principal.school_id)
    if current and current.status in ACTIVE_STATUSES: raise HTTPException(status.HTTP_409_CONFLICT,"A timetable is already being generated.")
    config=_config(db,principal.school_id,s.GenerateProfileIn(max_seconds=payload.max_seconds,timetable_type_id=payload.timetable_type_id,project_id=payload.project_id,class_ids=payload.class_ids,teacher_ids=payload.teacher_ids,period_indexes=payload.period_indexes)); job=job_queue.create_job(db,principal.school_id,principal.email,config); job_queue.enqueue(job.id,principal.school_id,payload.max_seconds,config["day_indexes"]); db.refresh(job); return job

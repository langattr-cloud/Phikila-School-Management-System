...
def _active_job(db, school_id):
    ...
    if job:
        ...
        stale = job.finished_at is not None or bool(
            heartbeat and now - heartbeat > timedelta(minutes=10)
        )
        if stale:
    ...
    recent = (
        ...
        .filter(
            m.TtSolverJob.school_id == school_id,
            m.TtSolverJob.status.in_(["completed", "failed", "cancelled"]),
        )
        .order_by(m.TtSolverJob.id.desc())
        .first()
    )
...
def _config(db, school_id, payload):
    ...
    if current is None:
        raise HTTPException(
            ...
            "Save a timetable type before generating a timetable.",
        )
    if payload.timetable_type_id is not None and int(payload.timetable_type_id) != int(current.id):
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "The selected timetable type is no longer current. Refresh and use the active type.",
        )
    ...
    configured_days = {
        int(d.index)
        for d in db.query(m.TtDay).filter(m.TtDay.school_id == school_id).all()
    }
    if not day_indexes or not set(day_indexes).issubset(configured_days):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            "The current timetable type contains days that are not configured.",
        )

    configured_periods = {
        int(p.index)
        for p in db.query(m.TtPeriod)
        .filter(
    ...
    if not period_indexes or not set(period_indexes).issubset(configured_periods):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            "The current timetable type contains periods that are not configured teaching periods.",
        )

    day_names = {
        int(d.index): str(d.name).strip()
        for d in db.query(m.TtDay).filter(m.TtDay.school_id == school_id).all()
        if int(d.index) in day_indexes and str(d.name).strip()
    }
    return {
        "label": payload.label,
        "timetable_type_id": current.id,
...
def generate_async(
    ...
    config = _config(
        ...
        s.GenerateProfileIn(
            ...
            timetable_type_id=payload.timetable_type_id,
            class_ids=payload.class_ids,
            teacher_ids=payload.teacher_ids,

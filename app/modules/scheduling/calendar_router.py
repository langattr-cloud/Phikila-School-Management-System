...
def calendar(db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin', 'scheduler'))):
    ...
    return {
        'days': [s.DayOut.model_validate(d).model_dump() for d in days],
        'periods': [s.PeriodOut.model_validate(p).model_dump() for p in periods],
        'display_mode': mode,
    }
...
def set_calendar(payload: CalendarIn, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin', 'scheduler'))):
    ...
    incoming_periods = {p.index: p for p in payload.periods}
    ...
    if not incoming_days or not incoming_periods:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, 'At least one day/date and one period are required.')

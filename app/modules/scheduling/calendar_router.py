...
def _rank_map(existing: dict[int, object], incoming: dict[int, object]) -> dict[int, int]:
    """Preserve explicit calendar indexes; do not remap by list position."""
    return {index: index for index in existing if index in incoming}
...
def _remap_json_slots(value, day_map: dict[int, int], period_map: dict[int, int]):
    ...
    for day, periods in value.items():
        ...
        if old_day is not None and old_day not in day_map: continue
        ...
        if isinstance(periods, (list, tuple, set)):
            ...
            for period in periods:
                ...
                if old_period in period_map: mapped.append(period_map[old_period])
...
def _remap_calendar_references(db: Session, school_id: int, day_map: dict[int, int], period_map: dict[int, int]) -> None:
    ...
    for version in db.query(m.TtVersion).filter(m.TtVersion.school_id == school_id).all():
        if isinstance(version.day_indexes, list): version.day_indexes = [day_map[int(v)] for v in version.day_indexes if int(v) in day_map]
    for timetable_type in db.query(m.TtTimetableType).filter(m.TtTimetableType.school_id == school_id).all():
        if isinstance(timetable_type.day_indexes, list): timetable_type.day_indexes = [day_map[int(v)] for v in timetable_type.day_indexes if int(v) in day_map]
        if isinstance(timetable_type.period_indexes, list): timetable_type.period_indexes = [period_map[int(v)] for v in timetable_type.period_indexes if int(v) in period_map]
    for event in db.query(m.TtEvent).filter(m.TtEvent.school_id == school_id).all():
        if isinstance(event.day_indexes, list): event.day_indexes = [day_map[int(v)] for v in event.day_indexes if int(v) in day_map]
...
def set_calendar(payload: CalendarIn, db: Session = Depends(get_db), principal: Principal = Depends(require_role('admin', 'scheduler'))):
    ...
    dropped_day_indexes = set(existing_days) - set(incoming_days)
    dropped_period_indexes = set(existing_periods) - set(incoming_periods)
    dropped_days = [existing_days[i] for i in sorted(dropped_day_indexes)]; dropped_periods = [existing_periods[i] for i in sorted(dropped_period_indexes)]
    if dropped_day_indexes:
        if db.query(m.TtLesson).filter(m.TtLesson.school_id == principal.school_id, m.TtLesson.day_index.in_(dropped_day_indexes)).first(): raise HTTPException(status.HTTP_409_CONFLICT, 'Cannot remove a day that contains timetable lessons. Move those lessons first.')
    if dropped_period_indexes:
        if db.query(m.TtLesson).filter(m.TtLesson.school_id == principal.school_id, m.TtLesson.period_index.in_(dropped_period_indexes)).first(): raise HTTPException(status.HTTP_409_CONFLICT, 'Cannot remove a period that contains timetable lessons. Move those lessons first.')
    ...
    temporary_base=1000
    for offset,current in enumerate(existing_days.values()): current.index=temporary_base+offset
    for offset,current in enumerate(existing_periods.values()): current.index=temporary_base+offset
    db.flush()
    for current in dropped_days+dropped_periods: db.delete(current)
    for day in new_days:
        current=existing_days.get(day.index); parsed=_parse_date(day.date_value or day.name) if payload.display_mode=='date' else None
    ...
    for period in new_periods:
        current=existing_periods.get(period.index); start=_as_time(period.start_time); end=_as_time(period.end_time)

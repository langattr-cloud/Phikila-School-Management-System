...
The scheduler uses hard feasibility constraints plus neutral quality objectives.
There is no time-of-day preference or preferred-period concept.
"""
...
def preflight(data: SolverInput) -> list[str]:
    ...
    if capacity == 0:
        ...
        return problems
    hard_class: dict[int, set[tuple[int, int]]] = {}
    ...
    for class_id, total in per_class.items():
        ...
        blocked = (set(spec.unavailable) if spec else set()) | hard_class.get(class_id, set())
        free = capacity - len(blocked)
        if total > free:
            problems.append(f"{spec.name if spec else f'Class {class_id}'} needs {total} lessons a week but only has {free} available slots.")
    ...
    for teacher_id, total in per_teacher.items():
        ...
        if not spec:
            continue
        blocked = set(spec.unavailable) | hard_teacher.get(teacher_id, set())
        limit = min(capacity - len(blocked), spec.max_per_day * len(data.days))
        if total > limit:
            problems.append(f"{spec.name} is assigned {total} lessons a week but can only teach {limit}.")
    for req in data.requirements:
        subject = data.subjects.get(req.subject_id)
        if subject and subject.required_room_type and not req.room_id:
            if not any(r.room_type == subject.required_room_type for r in data.rooms.values()):
                problems.append(f"{subject.name} requires a '{subject.required_room_type}' room but no such room exists.")
    return problems
...
def solve(data: SolverInput, on_progress: Callable[[int, str], None] | None = None, should_cancel: Callable[[], bool] | None = None) -> SolverOutput:
    ...
    def allowed(req: RequirementSpec, day: int, period: int) -> bool:
        ...
        if klass and (day, period) in klass.unavailable:
            return False
        if req.teacher_id and (teacher := data.teachers.get(req.teacher_id)) and (day, period) in teacher.unavailable:
            return False
        if req.room_id and (room := data.rooms.get(req.room_id)) and (day, period) in room.unavailable:
            return False
        for rule in data.avoid_rules:
    ...
    for req in data.requirements:
        ...
        if len(vars_for_req) < req.periods_per_week:
            return SolverOutput("infeasible", [], {}, {}, [f"Requirement {req.id} needs {req.periods_per_week} periods but only {len(vars_for_req)} valid slots exist."])
    ...
    for class_id in data.classes:
        ...
        for day, period in slots:
            values = [x[(r.id, day, period)] for r in reqs if (r.id, day, period) in x]
            if len(values) > 1:
                model.AddAtMostOne(values)
    ...
    for room_id in data.rooms:
        ...
        for day, period in slots:
            ...
            if len(values) > 1:
                model.AddAtMostOne(values)
    for room_type, count in {t: sum(1 for r in data.rooms.values() if r.room_type == t) for t in {r.room_type for r in data.rooms.values() if r.room_type != "classroom"}}.items():
        for day, period in slots:
            demand = [x[(r.id, day, period)] for r in data.requirements if (r.id, day, period) in x and data.subjects.get(r.subject_id) and data.subjects[r.subject_id].required_room_type == room_type]
            if len(demand) > count:
    ...
    for teacher_id, spec in data.teachers.items():
        ...
        for day in data.days:
            values = [x[(r.id, day, p)] for r in reqs for p in data.teaching_periods if (r.id, day, p) in x]
            if values:
                model.Add(sum(values) <= spec.max_per_day)
    for req in data.requirements:
        subject = data.subjects.get(req.subject_id)
        if not subject or not subject.spread_across_week or req.periods_per_week > len(data.days):
            continue
        cap = 2 if req.double_periods else 1
        for day in data.days:
            values = [x[(req.id, day, p)] for p in data.teaching_periods if (req.id, day, p) in x]
            if values:
                model.Add(sum(values) <= cap)
    ...
    if w.teacher_gaps > 0:
        for teacher_id in data.teachers:
            for day in data.days:
                row = [busy[(teacher_id, day, p)] for p in data.teaching_periods]
                for i in range(1, len(row) - 1):
                    gap = model.NewBoolVar(f"gap_{teacher_id}_{day}_{i}")
                    model.Add(gap <= row[i - 1]); model.Add(gap <= row[i + 1]); model.Add(gap <= 1 - row[i]); model.Add(gap >= row[i - 1] + row[i + 1] - row[i] - 1)
        ...
        for teacher_id, spec in data.teachers.items():
            ...
            for day in data.days:
                ...
                for start in range(max(0, len(row) - run)):
                    ...
                    model.Add(sum(window) - run <= over * len(window)); model.Add(over <= sum(window)); penalties.append((over, w.consecutive_lessons))
    if w.subject_distribution > 0:
        for req in data.requirements:
            ...
            for day in data.days:
                values = [x[(req.id, day, p)] for p in data.teaching_periods if (req.id, day, p) in x]
                if len(values) > 1:
                    excess = model.NewIntVar(0, len(values), f"clump_{req.id}_{day}")
                    model.Add(excess >= sum(values) - 1); penalties.append((excess, w.subject_distribution))
    if w.avoid_slots > 0:
        for rule in data.avoid_rules:
    ...
    if w.workload_balance > 0:
        for teacher_id in data.teachers:
            ...
            for day in data.days:
                values = [x[(r.id, day, p)] for r in reqs for p in data.teaching_periods if (r.id, day, p) in x]
                if values:
                    over = model.NewIntVar(0, len(values), f"load_{teacher_id}_{day}")
                    model.Add(over >= sum(values) - fair); penalties.append((over, w.workload_balance))
    if penalties: model.Minimize(sum(var * weight for var, weight in penalties))
    report(38, "Optimising")
    solver = cp_model.CpSolver(); solver.parameters.max_time_in_seconds = float(data.max_seconds); solver.parameters.num_search_workers = int(data.workers)
    callback = _ProgressCallback(report, should_cancel); status = solver.Solve(model, callback)
    if should_cancel and should_cancel(): return SolverOutput("cancelled", [], {}, {}, ["Generation cancelled."])
    if status == cp_model.OPTIMAL: label = "optimal"
    elif status == cp_model.FEASIBLE: label = "feasible"
    else: return SolverOutput("infeasible", [], {}, {}, ["No timetable satisfies every hard constraint. Relax an availability rule, reduce weekly lessons, or add rooms/periods."])
    report(84, "Validating")
    placements = [Placement(req.id, req.class_id, req.subject_id, req.teacher_id, req.room_id, day, period) for req in data.requirements for day, period in slots if (req.id, day, period) in x and solver.Value(x[(req.id, day, period)])]
    quality = score(data, placements)
    stats = {"placed": len(placements), "required": sum(r.periods_per_week for r in data.requirements), "conflicts": 0, "penalty": int(solver.ObjectiveValue()) if penalties else 0, "wall_time": round(solver.WallTime(), 2), "status": label}
...
def score(data: SolverInput, placements: Sequence[Placement]) -> dict:
    required = sum(r.periods_per_week for r in data.requirements); placed = len(placements)
    ...
    for periods in by_teacher_day.values():
        periods.sort()
        if len(periods) > 1:
            span = periods[-1] - periods[0] + 1; spans += span; gaps += span - len(periods)
    gap_score = 100.0 if spans == 0 else round(100.0 * (1 - gaps / spans), 1)
    by_class_subject_day: dict[tuple[int, int, int], int] = {}
    for p in placements: by_class_subject_day[(p.class_id, p.subject_id, p.day)] = by_class_subject_day.get((p.class_id, p.subject_id, p.day), 0) + 1
    repeats = sum(v - 1 for v in by_class_subject_day.values() if v > 1)
    dist_score = 100.0 if placed == 0 else round(max(0.0, 100.0 * (1 - repeats / placed)), 1)
    capacity = len(data.days) * len(data.teaching_periods) * max(1, len(data.rooms)); used = sum(1 for p in placements if p.room_id is not None)
    ...
    breakdown = {"hard_constraints": hard, "teacher_workload": workload_score, "subject_distribution": dist_score, "room_utilisation": room_score, "teacher_gaps": gap_score, "class_distribution": class_score}
    overall = hard * 0.40 + workload_score * 0.14 + dist_score * 0.16 + gap_score * 0.14 + class_score * 0.10 + room_score * 0.06

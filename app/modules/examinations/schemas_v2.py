...
class SeriesCreate(BaseModel): name: str = Field(min_length=1, max_length=100); academic_year_id: int | None = None; term_id: int | None = None
class SeriesUpdate(BaseModel): name: str | None = Field(default=None, min_length=1, max_length=100); academic_year_id: int | None = None; term_id: int | None = None
...
class ExaminationCreate(BaseModel):
    series_id: int; name: str = Field(min_length=1, max_length=150); description: str | None = None; exam_date: date | None = None; total_marks: int = Field(default=100, ge=1); passing_marks: int = Field(default=50, ge=0)
    @model_validator(mode="after")
    def validate_marks(self):
        if self.passing_marks > self.total_marks: raise ValueError("Passing marks cannot be greater than total marks.")
        return self
class ExaminationUpdate(BaseModel):
    name: str | None = Field(default=None, min_length=1, max_length=150); description: str | None = None; exam_date: date | None = None; total_marks: int | None = Field(default=None, ge=1); passing_marks: int | None = Field(default=None, ge=0)
    @model_validator(mode="after")
    def validate_marks(self):
        if self.total_marks is not None and self.passing_marks is not None and self.passing_marks > self.total_marks: raise ValueError("Passing marks cannot be greater than total marks.")
        return self
...
class GradeScaleResponse(BaseModel):
    ...
    model_config = {"from_attributes": True}
class StatusChange(BaseModel): status: str = Field(pattern=r"^(draft|active|published|locked)$")

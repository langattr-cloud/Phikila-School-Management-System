...
def _series(db, school_id, series_id):
    ...
    if not row: raise HTTPException(status.HTTP_404_NOT_FOUND, "Examination series not found.")
    return row
...
def _assert_mutable(exam):
    if exam.status in ("published", "locked"): raise HTTPException(status.HTTP_409_CONFLICT, "Published or locked examinations cannot be changed.")
...
def create_examination(payload: s.ExaminationCreate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin"))):
    _series(db, principal.school_id, payload.series_id); x = m.ExaminationV2(school_id=principal.school_id, **payload.model_dump()); db.add(x); db.commit(); db.refresh(x); return x
...
def update_examination(exam_id: int, payload: s.ExaminationUpdate, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin"))):
    x = _exam(db, principal.school_id, exam_id); _assert_mutable(x); data = payload.model_dump(exclude_unset=True); total = data.get("total_marks", x.total_marks); passing = data.get("passing_marks", x.passing_marks)
...
def change_examination_status(exam_id: int, payload: s.StatusChange, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    ...
    if payload.status != x.status and payload.status not in allowed.get(x.status, set()): raise HTTPException(status.HTTP_409_CONFLICT, f"Cannot change examination status from {x.status} to {payload.status}.")
...
def list_exam_subjects(exam_id: int, db: Session = Depends(get_db), principal: Principal = Depends(require_role("viewer", "teacher", "admin"))):
    _exam(db, principal.school_id, exam_id); return db.query(m.ExamSubject).filter(m.ExamSubject.school_id == principal.school_id, m.ExamSubject.exam_id == exam_id).all()
...
def assign_exam_subject(exam_id: int, payload: ExamSubjectAssignment, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    exam = _exam(db, principal.school_id, exam_id); _assert_mutable(exam); _context(db, principal.school_id, payload.academic_year_id, payload.level_id, payload.grade_id, payload.stream_id); _teacher(db, payload.teacher_id)
...
def update_exam_subject_assignment(exam_id: int, assignment_id: int, payload: ExamSubjectAssignment, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler"))):
    exam = _exam(db, principal.school_id, exam_id); _assert_mutable(exam); x = db.query(m.ExamSubject).filter(m.ExamSubject.id == assignment_id, m.ExamSubject.exam_id == exam_id, m.ExamSubject.school_id == principal.school_id).first()
    if not x: raise HTTPException(status.HTTP_404_NOT_FOUND, "Examination subject assignment not found.")
    _context(db, principal.school_id, payload.academic_year_id, payload.level_id, payload.grade_id, payload.stream_id); _teacher(db, payload.teacher_id); d = _assignment(db, principal.school_id, exam_id, payload.subject_id, payload.academic_year_id, payload.grade_id, payload.stream_id)
...
def enter_scores(exam_id: int, payload: s.BulkScoreEntry, db: Session = Depends(get_db), principal: Principal = Depends(require_role("admin", "scheduler", "teacher"))):
    exam = _exam(db, principal.school_id, exam_id); _assert_mutable(exam); assignments = {r.subject_id: r for r in db.query(m.ExamSubject).filter(m.ExamSubject.exam_id == exam_id, m.ExamSubject.school_id == principal.school_id).all()}

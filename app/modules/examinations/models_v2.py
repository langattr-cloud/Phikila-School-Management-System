...
class ExamEntry(Base):
    ...
    score = Column(Float); grade = Column(String(5)); position = Column(Integer); remarks = Column(Text); entered_by = Column(String(64)); created_at = Column(DateTime(timezone=True), server_default=func.now()); updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class GradeScale(Base):
    ...
    id = Column(Integer, primary_key=True, index=True); school_id = Column(Integer, primary_key=False, nullable=False, index=True); grade = Column(String(5), nullable=False); min_score = Column(Float, nullable=False); max_score = Column(Float, nullable=False); points = Column(Integer); description = Column(String(100)); education_level = Column(String(20), nullable=True)
